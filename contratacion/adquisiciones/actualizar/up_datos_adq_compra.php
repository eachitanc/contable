...
if ($id_pretbnsv === $id_posttbnsv) {
    ...
    try {
        ...
        $sql = "DELETE FROM seg_detalle_adquisicion  WHERE id_adquisicion = ?";
        $sql = $cmd->prepare($sql);
...
function upCompra()
{
    try {
        ...
        $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
        $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
        $sql = "UPDATE `seg_adquisiciones` SET `id_modalidad` = ?, `fecha_adquisicion` = ?, `val_contrato` = ?, `id_tipo_bn_sv` = ?, `objeto` = ? WHERE `id_adquisicion` = ?";
        $sql = $cmd->prepare($sql);
