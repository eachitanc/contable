...
try {
    ...
    $sql = "UPDATE `seg_estudios_previos` SET `fec_ini_ejec` = ?, `fec_fin_ejec` = ?, `val_contrata` = ?, `id_forma_pago` = ?, `id_supervisor` = ?
    ...
    if (!($sql->execute())) {
        ...
        if ($cambio > 0) {
            $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
            $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
            $sql = "UPDATE  `seg_estudios_previos` SET  `id_user_act` = ? , `fec_act` = ? WHERE `id_est_prev` = ?";
            $sql = $cmd->prepare($sql);
