...
try {
    ...
    $sql = "UPDATE `seg_contrato_compra` SET `fec_ini` = ?, `fec_fin` = ?, `id_forma_pago` = ?, `id_supervisor` = ? WHERE `id_contrato_compra` = ?";
    $sql = $cmd->prepare($sql);
    ...
    if (!($sql->execute())) {
        ...
        try {
            $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
            $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
            $sql = "DELETE FROM `seg_garantias_contrato_compra` WHERE `id_contrato_compra` = '$id_cc'";
            $sql = $cmd->prepare($sql);
        ...
        if ($polizas == '') {
            ...
            try {
                $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
                $sql = "INSERT INTO `seg_garantias_contrato_compra`(`id_contrato_compra`,`id_poliza`,`id_user_reg`,`fec_reg`) VALUES (?, ?, ?, ?)";
                $sql = $cmd->prepare($sql);
                $sql->bindParam(1, $id_cc, PDO::PARAM_INT);
        ...
        if ($cambio > 0) {
            $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
            $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
            $sql = "UPDATE  `seg_contrato_compra` SET  `id_user_act` = ? , `fec_act` = ? WHERE `id_contrato_compra` = ?";
            $sql = $cmd->prepare($sql);
