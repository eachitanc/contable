...
try {
    ...
    $sql = "SELECT `id_adquisicion`, `estado` FROM `seg_adquisiciones` WHERE `estado` = '3' AND `vigencia` = '$vigencia'";
    $rs = $cmd->query($sql);
...
if ($_SESSION['id_user'] == 1) {
    ...
    $usuario = " AND seg_adquisiciones.id_user_reg =" . $id_user;
}
try {
    $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
    $sql = "SELECT modalidad, id_adquisicion, val_contrato, seg_adquisiciones.estado, fecha_adquisicion, objeto, id_tercero_api
            FROM
                seg_adquisiciones
...
if (!empty($ladquis)) {
    foreach ($ladquis as $la) {
        ...
        if ((intval($permisos['borrar'])) == 1 && $la['estado'] <= 2) {
