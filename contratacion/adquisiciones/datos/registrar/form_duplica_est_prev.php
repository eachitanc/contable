...
try {
    ...
    $sql = "SELECT
                ...
                , `seg_adquisiciones`.`id_area`
                , `seg_adquisiciones`.`fecha_adquisicion`
                , `seg_adquisiciones`.`val_contrato`
                , `seg_adquisiciones`.`id_tipo_bn_sv`
                , `seg_adquisiciones`.`vigencia`
            ...
            WHERE (`seg_adquisiciones`.`id_adquisicion` = $id_aquisicion)";
    $rs = $cmd->query($sql);
...
try {
    $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
    $sql = "SELECT `id_area`, `area` FROM `seg_area_c` ORDER BY `area` ASC";
    $rs = $cmd->query($sql);
    $areas = $rs->fetchAll();
    $cmd = null;
} catch (PDOException $e) {
            ...
            <div id="contenedor">
                ...
                foreach ($destino as $des) {
                    ...
                    try {
                        $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                        $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
                        $sql = "SELECT `id_sede` FROM `seg_centro_costo_x_sede` WHERE `id_x_sede` = $id_cc";
                        $rs = $cmd->query($sql);
                        $cencos = $rs->fetch();
                        $cmd = null;
                    } catch (PDOException $e) {
