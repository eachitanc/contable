...
try {
    ...
    $sql = "SELECT
            ...
            FROM
                `seg_metodo_pago` ORDER BY `metodo` ASC";
    $rs = $cmd->query($sql);
    $metodop = $rs->fetchAll();
    $cmd = null;
} catch (PDOException $e) {
...
try {
    ...
    $sql = "SELECT
            ...
            FROM
                `seg_tipos_documento` ORDER BY `descripcion` ASC";
    $rs = $cmd->query($sql);
    $tip_doc = $rs->fetchAll();
    $cmd = null;
} catch (PDOException $e) {
...
try {
    ...
    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
    $sql = "SELECT
                `id_pais`, `codigo_pais`, `nombre_pais`
            FROM
                `seg_pais`";
    $rs = $cmd->query($sql);
    $pais = $rs->fetchAll();
...
try {
    ...
    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
    $sql = "SELECT
                `id_dpto`, `id_pais`, `codigo_dpto`, `nombre_dpto`
            FROM
                `seg_departamento` ORDER BY `nombre_dpto` ASC";
    $rs = $cmd->query($sql);
    $dpto = $rs->fetchAll();
    $cmd = null;
} catch (PDOException $e) {
...
try {
    ...
    $sql = "SELECT 
            ...
            FROM 
                `seg_codificacion_unspsc` ORDER BY `descripcion` ASC";
    $rs = $cmd->query($sql);
                ...
                <div class="form-group col-md-2">
                    ...
                    <select id="slcPaisEmp" name="slcPaisEmp" class="form-control form-control-sm py-0 sm" aria-label="Default select example">
                        ...
                        foreach ($pais as $p) {
                            ...
                            echo '<option value="' . $p['id_pais'] . '" ' . $slc . '>' . $p['nombre_pais'] . '</option>';
                        }
                        ?>
                    </select>
                </div>
