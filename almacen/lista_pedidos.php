...
$tipo_contrato = 1; //17;
$tipo_bien = isset($_POST['tipo_bien']) ? $_POST['tipo_bien'] : 0;
try {
    $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
    $sql = "SELECT
                `id_tipo_b_s`, `id_tipo_cotrato`, `tipo_bn_sv`, `objeto_definido`
            FROM
                `seg_tipo_bien_servicio`
            WHERE `id_tipo_cotrato` = $tipo_contrato";
    $rs = $cmd->query($sql);
    $tbien = $rs->fetchAll();
    $cmd = null;
                ...
                <div class="container-fluid p-2">
                    <div class="card mb-4">
                        ...
                        <div class="card-body" id="divCuerpoPag">
                            ...
                            <input type="hidden" id="txtTipoBien" value="<?php echo $tipo_bien ?>">
                            <?php
                            if ($tipo_bien != 0 || $tipo_bien == "A") {
                            ?>
                                <table id="tablePedidos" class="table table-striped table-bordered table-sm nowrap table-hover shadow" style="width:100%">
                                    <thead>
