...
try {
    ...
    $sql = "SELECT `id_adquisicion`, `entregas` FROM `seg_adquisiciones` WHERE `id_adquisicion` = '$id_adq'";
    $rs = $cmd->query($sql);
            ...
            <table class="table-striped table-bordered table-sm nowrap table-hover shadow" width="100%">
                ...
                <tbody id="detallesXEntrega">
                    ...
                    for ($i = 1; $i <= $compra_entregada['num_entregas']['entregas']; $i++) {
                        ...
                        $detalles = $i > $cant_entregas['entregas'] ? '<a value="' . $id_c . '|' . $i . '" class="btn btn-outline-warning btn-sm btn-circle shadow-gb confirmar_entrega_almacen" title="Confirmar Entrada"><span class="fas fa-exclamation-triangle fa-lg"></span></a>' : 'REGISTRADO';
