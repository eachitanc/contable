...
if (isset($_POST)) {
    ...
    if (empty($_POST['id'])) {
        $query = $cmd->prepare("INSERT INTO seg_fin_fecha (vigencia,id_usuario,fecha) VALUES (?, ?, ?)");
        ...
        $id = $_POST['id'];
        $query = $cmd->prepare("UPDATE seg_fin_fecha SET vigencia = ?, id_usuario = ?, fecha =? WHERE id = ?");
        $query->bindParam(1, $vigencia, PDO::PARAM_INT);
        $query->bindParam(2, $usuario, PDO::PARAM_INT);
        $query->bindParam(3, $fecha, PDO::PARAM_STR);
