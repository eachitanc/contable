...
try {
    $sql = "SELECT 
            ...
            FROM seg_ctb_doc 
            INNER JOIN seg_usuarios ON (seg_ctb_doc.id_user_reg = seg_usuarios.id_usuario)
...
try {
    $sql = "SELECT nombre FROM seg_ctb_fuente WHERE cod ='$cdp[tipo_doc]'";
    $res = $cmd->query($sql);
    $tipo_doc = $res->fetch();
...
try {
    $sql = "SELECT id_crp  FROM seg_ctb_libaux WHERE id_ctb_doc =$dto limit 1";
    $res = $cmd->query($sql);
...
WHERE (`seg_pto_mvto`.`id_ctb_doc` =$dto
    AND `seg_pto_cargue`.`vigencia` =$vigencia);";
        $res = $cmd->query($sql);
        $rubros = $res->fetchAll();
    } catch (PDOException $e) {
        echo $e->getCode() == 2002 ? 'Sin Conexión a Mysql (Error: 2002)' : 'Error: ' . $e->getCode();
    ...
    try {
        $sql = "SELECT `id_ctb_cop` FROM `seg_pto_mvto` WHERE (`id_ctb_doc` =$dto) GROUP BY `id_ctb_cop`;";
        $rs = $cmd->query($sql);
...
try {
    ...
    WHERE (`seg_fin_respon_doc`.`id_maestro_doc` =5
    ...
    <div class="px-2 " style="width:90% !important;margin: 0 auto;">
        ...
        ?>
            ...
            foreach ($documentos as $doc) {
                ...
                try {
                    $sql = "SELECT id_manu FROM `seg_ctb_doc` WHERE `id_ctb_doc` ={$doc['id_ctb_cop']};";
                    $rs = $cmd->query($sql);
                    $causa = $rs->fetch();
