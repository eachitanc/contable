...
if (isset($_POST['id_doc'])) {
    try {
        $sql = "SELECT id_ctb_doc,fecha,id_manu,detalle,id_tercero,tipo_doc,id_ref FROM seg_ctb_doc WHERE id_ctb_doc=$id_doc";
        $rs = $cmd->query($sql);
        $datosMov = $rs->fetch();
    ...
    try {
        $sql = "SELECT sum(debito) as debito, sum(credito) as credito FROM seg_ctb_libaux WHERE id_ctb_doc=$id_doc GROUP BY id_ctb_doc";
        $rs = $cmd->query($sql);
        $sumaMov = $rs->fetch();
    ...
    // Consulto el valor de id_pto_doc en seg_pto_mvto cuando id_ctb_doc es igual a $id_doc
    try {
        $sql = "SELECT id_pto_doc,id_ctb_cop FROM seg_pto_mvto WHERE id_ctb_doc=$id_doc";
        $rs = $cmd->query($sql);
        $datosCrp = $rs->fetch();
...
if (isset($_POST['id_cop'])) {
    ...
    WHERE (`seg_pto_mvto`.`id_ctb_doc` =$id_cop);";
        $rs = $cmd->query($sql);
        $datosCrp = $rs->fetch();
    ...
    // consulto el objeto de la tabla seg_ctb_doc cuando id_ctb_doc = a $datosCrp['id_ctb_doc']
    try {
        $sql = "SELECT detalle FROM seg_ctb_doc WHERE id_ctb_doc={$datosCrp['id_ctb_doc']};";
        $rs = $cmd->query($sql);
...
// consulto el valor causado valor en la tabla seg_pto_mvto
try {
    $sql = "SELECT sum(valor) as valor FROM seg_pto_mvto WHERE id_ctb_doc=$id_doc AND tipo_mov='PAG'";
    $rs = $cmd->query($sql);
...
try {
    $sql = "SELECT
            ...
            FROM
                `seg_ctb_referencia`
                INNER JOIN `seg_ctb_fuente` 
                    ON (`seg_ctb_referencia`.`id_ctb_fuente` = `seg_ctb_fuente`.`id_doc_fuente`)
            WHERE (`seg_ctb_fuente`.`cod` ='$tipo_dato');";
    $rs = $cmd->query($sql);
...
if ($_POST['tipo_dato'] == 'CTCB') {
    ...
    try {
        $sql = "SELECT
                ...
                FROM
                    `seg_ctb_doc`
                WHERE `seg_ctb_doc`.`id_ctb_doc` =$id_arq;";
        echo $sql;
        $rs = $cmd->query($sql);
