...
try {
    ...
    $sql = "SELECT sum(debito) as debito, sum(credito) as credito FROM seg_ctb_libaux WHERE id_ctb_doc=$data GROUP BY id_ctb_doc";
    $rs = $cmd->query($sql);
    $sumaMov = $rs->fetch();
    ...
    // Consulto el tipo_doc de la tabla seg_ctb_doc
    $sql = "SELECT tipo_doc FROM seg_ctb_doc WHERE id_ctb_doc=$data LIMIT 1";
    $rs = $cmd->query($sql);
    $datos = $rs->fetch();
    ...
    if ($dif == 0) {
        ...
        $query = $cmd->prepare("UPDATE seg_ctb_doc SET estado=$estado WHERE id_ctb_doc=?");
        $query->bindParam(1, $data, PDO::PARAM_INT);
        $query->execute();
        // Actualizo el campo estado de la tabla seg_pto_mvto
        $query = $cmd->prepare("UPDATE seg_pto_mvto SET estado=0 WHERE id_ctb_doc=?");
        $query->bindParam(1, $data, PDO::PARAM_INT);
        $query->execute();
