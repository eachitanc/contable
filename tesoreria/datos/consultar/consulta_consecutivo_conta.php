...
$sql = "SELECT max(id_manu) as id_manu from seg_ctb_doc WHERE CAST(fecha AS DATE) <= '$_post[fecha]' AND tipo_doc='$_post[documento]' ";
...
do {
    $id_manu = $id_manu + 1;
    $sql = "SELECT id_manu from seg_ctb_doc WHERE id_manu='$id_manu' AND tipo_doc='$_post[documento]' ";
    $res = $conexion->query($sql);
    $num = $res->num_rows;
    if ($num == 0) {
        $i = 2;
