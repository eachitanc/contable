...
if (isset($_POST)) {
    ...
    try {
        $sql = "SELECT `id_ctb_cop` FROM `seg_pto_mvto` WHERE (`id_ctb_doc` =$id_doc) GROUP BY `id_ctb_cop`;";
        $rs = $cmd->query($sql);
        ...
        $sql = "SELECT
                ...
                FROM
                    ...
                    INNER JOIN `seg_ctb_referencia` 
                        ON (`seg_ctb_doc`.`id_ref` = `seg_ctb_referencia`.`id_ctb_referencia`)
                WHERE (`seg_ctb_doc`.`id_ctb_doc` =$id_doc);";
        $rs = $cmd->query($sql);
        $cuenta_ctb = $rs->fetch();
    } catch (PDOException $e) {
        echo $e->getCode() == 2002 ? 'Sin Conexión a Mysql (Error: 2002)' : 'Error: ' . $e->getCode();
    ...
    if ($cuenta_ctb != null) {
        ...
        if ($cuenta_ctb['accion'] == 1) {
            ...
            // Consulto la cuenta registrada en seg_ctb_libaux cuando id_factura > 0
            $sql = $cmd->prepare("SELECT
        ...
        foreach ($documentos as $des) {
            ...
            // Consulto la cuenta registrada en seg_ctb_libaux cuando id_factura > 0
            $sql = $cmd->prepare("SELECT cuenta,credito,id_fac,id_sede,id_cc FROM seg_ctb_libaux WHERE id_fac >0 AND id_ctb_doc= ?;");
