...
if (isset($_POST)) {
    ...
    $query = $cmd->prepare("SELECT tipo_doc FROM seg_ctb_doc WHERE id_ctb_doc=?");
    ...
    if ($cmd->lastInsertId() > 0) {
        ...
        $query = $cmd->prepare("UPDATE seg_ctb_doc SET estado=5 WHERE id_ctb_doc=?");
        $query->bindParam(1, $id_ctb_doc, PDO::PARAM_INT);
        $query->execute();
        // realiza un update al campo estado de la tabla seg_pto_mvto
        $query = $cmd->prepare("UPDATE seg_pto_mvto SET estado=5 WHERE id_ctb_doc=?");
        $query->bindParam(1, $id_ctb_doc, PDO::PARAM_INT);
        $query->execute();
