...
if (isset($_POST)) {
    ...
    try {
        ...
        if ($_POST['id_ctb_doc'] < 1) {
            ...
            $query = $cmd->prepare("INSERT INTO seg_ctb_doc (vigencia,id_sede, tipo_doc, id_manu,id_tercero, fecha, detalle, id_plano,id_ref, id_user_reg, fec_reg) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?,?,?)");
            ...
            $id = $_POST['id_ctb_doc'];
            $query = $cmd->prepare("UPDATE seg_ctb_doc SET id_manu = ?,id_tercero=?, fecha = ?, detalle =?, id_plano=?, id_ref=?, id_usuer_act=?,fec_act=? WHERE id_ctb_doc = ?");
