...
if (isset($_POST)) {
    ...
    try {
        ...
        $sql = "SELECT id_auto_dep,id_pto_doc FROM seg_pto_mvto WHERE id_ctb_doc = '$id_crrp' AND tipo_mov = 'COP'";
        $rs = $cmd->query($sql);
        $idCdp = $rs->fetch();
        $id_auto_cdp = $idCdp['id_auto_dep'];
        $id_rp = $idCdp['id_pto_doc'];
        if (empty($_POST['id'])) {
            $query = $cmd->prepare("INSERT INTO seg_pto_mvto (id_pto_doc,id_ctb_doc,id_auto_dep,tipo_mov, rubro, valor,estado,id_ctb_cop) VALUES (?, ?, ?, ?, ?, ?, ?,?)");
            ...
            foreach ($obj as $key => $value) {
                ...
                $sql = "SELECT rubro FROM seg_pto_mvto WHERE id_pto_mvto = '$id_rubro'";
                $query2 = $cmd->prepare($sql);
                $query2->execute();
                $row = $query2->fetch();
                $rubro = $row['rubro'];
                // consultar saldo del rubro, si saldo es suficiente realizar el registro sino eliminar documento y 
                $sq2 = "SELECT sum(valor) as obligado FROM seg_pto_mvto WHERE rubro = '$rubro' AND id_ctb_doc = $id_crrp  AND tipo_mov ='COP'";
                $rs2 = $cmd->query($sq2);
                $obligado = $rs2->fetch();
                $valor_obligado = $obligado['obligado'];
                $sq3 = "SELECT sum(valor) as pagado FROM seg_pto_mvto WHERE rubro = '$rubro' AND id_ctb_doc = $id_crrp  AND tipo_mov ='PAG'";
                $rs3 = $cmd->query($sq3);
                $pagado = $rs3->fetch();
            ...
            try {
                $sql = "SELECT sum(valor) as valorpag FROM seg_pto_mvto WHERE id_ctb_doc =$id_ctb_doc AND tipo_mov='PAG'";
                $rs = $cmd->query($sql);
                $sumacrp = $rs->fetch();
