...
if (isset($_POST)) {
    ...
    try {
        if (empty($_POST['id_editar'])) {
            $query = $cmd->prepare("INSERT INTO seg_ctb_libaux (id_ctb_doc,cuenta,debito,credito,id_crp,id_tercero,id_user_reg,fec_reg) VALUES (?, ?, ?, ?, ?, ?, ?,?)");
            ...
            $query = $cmd->prepare("UPDATE seg_ctb_libaux SET cuenta=?, debito=?, credito=?, id_usuer_act=?, fec_act=? WHERE id_ctb_libaux = ?");
