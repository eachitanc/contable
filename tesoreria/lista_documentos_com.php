...
try {
    ...
    $sql = "SELECT cod,nombre FROM seg_ctb_fuente WHERE tesor=$tipo";
    $sql3 = $sql;
    $rs = $cmd->query($sql);
...
try {
    ...
    $sql = "SELECT
            ...
            WHERE (`seg_nominas`.`estado` = 4) AND`seg_nomina_pto_ctb_tes`.`tipo` <> 'PL'
            UNION 
            SELECT
                `seg_nomina_pto_ctb_tes`.`id`
                , `seg_nomina_pto_ctb_tes`.`id_nomina`
            ...
            FROM
                ...
                INNER JOIN `seg_nominas` 
                    ON (`seg_nomina_pto_ctb_tes`.`id_nomina` = `seg_nominas`.`id_nomina`)
            WHERE (`seg_nominas`.`planilla` = 4 AND `seg_nomina_pto_ctb_tes`.`tipo` = 'PL')";
    $rs = $cmd->query($sql);
