...
try {
    $sql = "SELECT DISTINCT
                `seg_ctb_libaux`.`cuenta`
                , `seg_ctb_pgcp`.`nombre`
                , SUBSTRING(`seg_ctb_libaux`.`cuenta`,1,$long_ini) AS cta
            FROM
                `seg_ctb_libaux`
            ...
            WHERE (`seg_ctb_doc`.estado =1 AND SUBSTRING(`seg_ctb_libaux`.`cuenta`,1,$long_ini) BETWEEN $cuenta_ini AND $cuenta_fin);";
    $res = $cmd->query($sql);
        ...
        foreach ($cuentas as $cta) {
            try {
                $sql = "SELECT
                            ...
                            , IF(LENGTH(`seg_ctb_libaux`.`id_tercero`) >=1,`seg_ctb_libaux`.`id_tercero`,`seg_ctb_doc`.`id_tercero`) AS id_tercero
                            , `seg_ctb_doc`.`detalle`
                        ...
                        FROM
                            ...
                            LEFT JOIN `seg_tes_detalle_pago` 
                                ON (`seg_tes_detalle_pago`.`id_ctb_doc` = `seg_ctb_doc`.`id_ctb_doc`)
                            LEFT JOIN `seg_tes_forma_pago` 
                                ON (`seg_tes_detalle_pago`.`id_forma_pago` = `seg_tes_forma_pago`.`id_forma_pago`)
                        WHERE `seg_ctb_libaux`.`cuenta` ='{$cta['cuenta']}' AND `seg_ctb_doc`.`estado`=1 
            ...
            try {
                $sql = "SELECT
                            `seg_ctb_libaux`.`cuenta`
                            , SUM(`seg_ctb_libaux`.`debito`) as debito
                            , SUM(`seg_ctb_libaux`.`credito`) as credito
                        FROM
                            `seg_ctb_libaux`
                            INNER JOIN `seg_ctb_doc` 
                                ON (`seg_ctb_libaux`.`id_ctb_doc` = `seg_ctb_doc`.`id_ctb_doc`)
                        WHERE (`seg_ctb_doc`.`fecha` <'$fecha_inicial' AND  `seg_ctb_libaux`.`cuenta`='{$cta['cuenta']}' AND `seg_ctb_doc`.`estado`=1 )
                        GROUP BY `seg_ctb_libaux`.`cuenta`, `seg_ctb_libaux`.`debito`, `seg_ctb_libaux`.`credito`;";
                $res = $cmd->query($sql);
                $saldo_ini = $res->fetch();
            } catch (PDOException $e) {
                echo $e->getCode() == 2002 ? 'Sin Conexión a Mysql (Error: 2002)' : 'Error: ' . $e->getCode();
