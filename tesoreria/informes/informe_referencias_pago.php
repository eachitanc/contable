...
try {
    $sql = "SELECT
            ...
            LEFT JOIN (
                SELECT 
                    SUM(seg_pto_mvto.valor) AS causado
                    ,id_ctb_cop
                FROM 
                    seg_pto_mvto 
                GROUP BY seg_pto_mvto.id_ctb_cop
            ) AS  causacion ON (seg_pto_mvto.id_ctb_cop =causacion.id_ctb_cop)
            LEFT JOIN(
                SELECT 
            ...
            ) AS descuentos ON (seg_pto_mvto.id_ctb_cop=descuentos.id_ctb_doc)
            WHERE (seg_ctb_doc.id_plano =$id);
";
    $res = $cmd->query($sql);
