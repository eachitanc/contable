...
if (isset($_POST)) {
    ...
    try {
        ...
        if ($_POST['id_ctb_doc'] < 1) {
            ...
            $sql = $cmd->prepare("SELECT `tipo_doc`, MAX(`id_manu`) AS id_manu FROM `seg_ctb_doc` WHERE (`tipo_doc` ='$tipodato');");
            $sql->execute();
            $datos = $sql->fetch(PDO::FETCH_ASSOC);
            // Consulto si 
            $sq2 = $cmd->prepare("SELECT id_manu FROM seg_ctb_doc WHERE tipo_doc='$tipodato' AND id_manu='$numDoc'");
            $sq2->execute();
            $consec = $sq2->fetch(PDO::FETCH_ASSOC);
            if ($consec['id_manu'] == $numDoc) {
                $numDoc = $datos['id_manu'] + 1;
            }
            $query = $cmd->prepare("INSERT INTO seg_ctb_doc (vigencia,id_sede, tipo_doc, id_manu,id_tercero, fecha, detalle, id_user_reg, fec_reg) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
            ...
            $id = $_POST['id_ctb_doc'];
            $query = $cmd->prepare("UPDATE seg_ctb_doc SET id_manu = ?, fecha = ?, detalle =?, id_usuer_act=?,fec_act=?,id_tercero=? WHERE id_ctb_doc = ?");
