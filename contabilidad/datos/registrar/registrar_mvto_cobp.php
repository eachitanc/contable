...
if (isset($_POST)) {
    ...
    try {
        ...
        $sql = "SELECT id_auto_dep FROM seg_pto_mvto WHERE id_pto_doc = '$id_crrp' AND tipo_mov = 'CRP'";
        $rs = $cmd->query($sql);
        $idCdp = $rs->fetch();
        ...
        if (empty($_POST['id'])) {
            ...
            foreach ($obj as $key => $value) {
                ...
                $sq2 = "SELECT sum(valor) as obligado FROM seg_pto_mvto WHERE rubro = '$rubro' AND id_ctb_doc = $id_ctb_doc  AND tipo_mov ='COP'";
                $rs2 = $cmd->query($sq2);
                $obligado = $rs2->fetch();
                $valor_obligado = $obligado['obligado'];
                $sq3 = "SELECT sum(valor) as comprometido FROM seg_pto_mvto WHERE rubro = '$rubro' AND id_pto_doc = $id_crrp AND tipo_mov ='CRP'";
                $rs3 = $cmd->query($sq3);
