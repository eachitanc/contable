...
if (isset($_POST)) {
    ...
    // Verifico si en la tabla seg_ctb_libaux existe un registro con el id_ctb_doc = $id_doc
    $query = $cmd->prepare("SELECT id_ctb_libaux FROM seg_ctb_libaux WHERE id_ctb_doc = ?;");
    $query->bindParam(1, $id_doc, PDO::PARAM_INT);
    $query->execute();
    $datos = $query->fetch();
    // verifico si $datos tiene registros
    if ($datos != null) {
        // eliminar todos los registros en seg_ctb_libaux donde id_ctb_doc = id_doc
        $query = $cmd->prepare("DELETE FROM seg_ctb_libaux WHERE id_ctb_doc = ?;");
        $query->bindParam(1, $id_doc, PDO::PARAM_INT);
        $query->execute();
    }
    // Consulto en la tabla seg_ctb_doc cuando id_ctb_doc = $id_doc
    $query = $cmd->prepare("SELECT id_tercero FROM seg_ctb_doc WHERE id_ctb_doc = ?;");
    $query->bindParam(1, $id_doc, PDO::PARAM_INT);
    $query->execute();
    $datos = $query->fetch();
    ...
    WHERE (`seg_pto_documento`.`id_pto_doc` =$id_crp);");
    ...
    $id_rte = 0;
    $id_fac = 0;
    $query = $cmd->prepare("INSERT INTO seg_ctb_libaux (id_ctb_doc,id_tercero,cuenta,debito,credito,id_sede,id_cc,id_crp,id_rte,id_fac,id_tipo_ad,id_user_reg,fec_reg) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)");
    $query->bindParam(1, $id_doc, PDO::PARAM_INT);
    ...
    foreach ($datoscostos as $key => $value) {
        ...
        $sql = $cmd->prepare("SELECT cuenta FROM seg_ctb_libaux WHERE id_tipo_ad = ? AND id_cc = ? ;");
    ...
    foreach ($datosretencion as $key => $value) {
        ...
        // Consultar el numero de cuenta para un registro similiar
        $sql = $cmd->prepare("SELECT cuenta FROM seg_ctb_libaux WHERE id_rte = ? ;");
    ...
    $sql = $cmd->prepare("SELECT SUM(debito) AS debito, SUM(credito) AS credito FROM seg_ctb_libaux WHERE id_ctb_doc = ? ;");
    ...
    $sql = $cmd->prepare("SELECT cuenta FROM seg_ctb_libaux WHERE id_tipo_ad = ? AND id_fac >0;");
    $sql->bindParam(1, $id_tipo_bn_sv, PDO::PARAM_INT);
    $sql->execute();
    $result = $sql->fetch();
    $cuenta = $result['cuenta'];
