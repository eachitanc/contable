...
if (isset($_POST['search'])) {
    ...
    $sq1 = "SELECT `id_dpto` FROM `seg_empresas`;";
    $re1 = $conexion->query($sq1);
    $row1 = $re1->fetch_assoc();
    $id_dpto = $row1['id_dpto'];

    $sql = "SELECT id_municipio,codigo_municipio,nom_municipio FROM seg_municipios WHERE id_departamento =$id_dpto AND (codigo_municipio LIKE '$search%' OR nom_municipio LIKE '$search%')";
