...
try {
    ...
    $res = $query->fetch();
    $cuenta = $res['cuenta'];
    $query = $pdo->prepare("SELECT id_ctb_libaux FROM seg_ctb_libaux WHERE cuenta = ?");
    ...
    if ($query->rowCount() > 0) {
        ...
        $query = $pdo->prepare("SELECT id_ctb_libaux FROM seg_ctb_libaux WHERE cuenta LIKE CONCAT(?, '%')");
        $query->bindParam(1, $cuenta);
        $query->execute();
        // consulto cuantos registros genera la sentencia
        if ($query->rowCount() > 0) {
