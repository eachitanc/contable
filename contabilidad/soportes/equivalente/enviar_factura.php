...
try {
    ...
    $sql = "SELECT
                ...
                , `seg_empresas`.`user_prov`
                , `seg_empresas`.`pass_prov`
            FROM
                `seg_empresas`
                INNER JOIN `seg_pais` 
                ...
                INNER JOIN `seg_responsabilidad_fiscal` 
                    ON (`seg_empresas`.`resp_fiscal` = `seg_responsabilidad_fiscal`.`id`)
            WHERE `seg_empresas`.`id_empresa` = 2";
    $rs = $cmd->query($sql);
    $empresa = $rs->fetch();
    $cmd = null;
} catch (PDOException $e) {
...
try {
    ...
    $sql = "SELECT
            ...
            WHERE (`seg_ctb_doc`.`id_ctb_doc` = $id_facno) LIMIT 1";
    $rs = $cmd->query($sql);
    $contab = $rs->fetch();
    $cmd = null;
} catch (PDOException $e) {
...
try {
    ...
    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
    $sql = "SELECT
                `seg_clasificacion_bn_sv`.`id_unspsc`
            FROM
                `seg_adquisiciones`
                INNER JOIN `seg_pto_mvto` 
                    ON (`seg_adquisiciones`.`id_cdp` = `seg_pto_mvto`.`id_auto_dep`)
                INNER JOIN `seg_ctb_factura` 
                    ON (`seg_pto_mvto`.`id_ctb_doc` = `seg_ctb_factura`.`id_ctb_doc`)
                INNER JOIN `seg_detalle_adquisicion` 
                    ON (`seg_detalle_adquisicion`.`id_adquisicion` = `seg_adquisiciones`.`id_adquisicion`)
                INNER JOIN `seg_clasificacion_bn_sv` 
                    ON (`seg_detalle_adquisicion`.`id_bn_sv` = `seg_clasificacion_bn_sv`.`id_b_s`)
...
try {
    $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
    $sql = "SELECT  `id_municipio`,`cod_postal` FROM `seg_municipios` WHERE `id_municipio` = $mun LIMIT 1";
    $rs = $cmd->query($sql);
    $postal = $rs->fetch();
    $cmd = null;
} catch (PDOException $e) {
...
$factura['nombre_dpto'] = $terceros[0]['nombre_dpto'];
...
try {
    ...
    $sql = "SELECT
            ...
            FROM
                `seg_resoluciones`
            WHERE `id_resol` = (SELECT MAX(`id_resol`) FROM `seg_resoluciones` WHERE `id_empresa` = '$id_empresa' AND `tipo` = 2)";
    $rs = $cmd->query($sql);
    $resolucion = $rs->fetch();
...
$jDocument = [
    ...
    'jseller' => [
        ...
        "sDepartmentName" => ucfirst(mb_strtolower($factura['nombre_dpto'])),
...
if ($resnom['rerror'] == 0) {
    ...
    try {
        ...
        if ($cmd->lastInsertId() > 0) {
            ...
            $query = "UPDATE `seg_resoluciones` SET `consecutivo` = '$secuenciaf'+1 WHERE `id_resol` = '$id_sec'";
            $rs = $cmd->query($query);
            $procesado++;
        } else {
            $err = $sql->errorInfo()[2];
