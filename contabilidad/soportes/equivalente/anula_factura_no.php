...
try {
    ...
    $sql = "SELECT `id_valxvig`, `id_concepto`, `valor`,`concepto`
            ...
            INNER JOIN `seg_vigencias` 
                ON (`seg_valxvigencia`.`id_vigencia` = `seg_vigencias`.`id_vigencia`)
...
try {
    ...
    $sql = "SELECT
                ...
                , `tb_pais`.`nombre_pais`
                , `tb_pais`.`codigo_pais`
                , `tb_departamento`.`codigo_dpto`
                , `tb_departamento`.`nombre_dpto`
            ...
            FROM
                `seg_empresas`
                INNER JOIN `tb_pais` 
                    ON (`seg_empresas`.`id_pais` = `tb_pais`.`id_pais`)
                INNER JOIN `tb_departamento` 
                    ON (`tb_departamento`.`id_pais` = `tb_pais`.`id_pais`) AND (`seg_empresas`.`id_dpto` = `tb_departamento`.`id_dpto`)
                INNER JOIN `tb_municipios` 
                    ON (`tb_municipios`.`id_departamento` = `tb_departamento`.`id_dpto`) AND (`seg_empresas`.`id_ciudad` = `tb_municipios`.`id_municipio`)
                INNER JOIN `tb_responsabilidad_fiscal` 
                    ON (`seg_empresas`.`resp_fiscal` = `tb_responsabilidad_fiscal`.`id`)
            WHERE `seg_empresas`.`id_empresa` = '$id_empresa'";
    $rs = $cmd->query($sql);
    $empresa = $rs->fetch();
    $cmd = null;
} catch (PDOException $e) {
...
try {
    ...
    $sql = "SELECT
            ...
            FROM
                ...
                INNER JOIN `tb_pais` 
                    ON (`tb_terceros_noblig`.`id_pais` = `tb_pais`.`id_pais`)
                INNER JOIN `tb_departamento` 
                    ON (`tb_departamento`.`id_pais` = `tb_pais`.`id_pais`) AND (`tb_terceros_noblig`.`id_dpto` = `tb_departamento`.`id_dpto`)
                INNER JOIN `tb_municipios` 
                    ON (`tb_municipios`.`id_departamento` = `tb_departamento`.`id_dpto`) AND (`tb_terceros_noblig`.`id_municipio` = `tb_municipios`.`id_municipio`)
