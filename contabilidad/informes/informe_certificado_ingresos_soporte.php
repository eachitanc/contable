...
try {
    $sql = "SELECT
            ...
            WHERE `seg_ctb_doc`.`id_tercero` =$id_tercero AND  `seg_ctb_doc`.`fecha` BETWEEN '$fecha_ini' AND '$fecha_fin' AND `seg_ctb_doc`.`tipo_doc` ='NCXP'  AND `seg_ctb_retencion_tipo`.`id_retencion_tipo` IN ($campos)
...
try {
    ...
    WHERE (`seg_fin_maestro_doc`.`tipo_doc` ='CIR'
