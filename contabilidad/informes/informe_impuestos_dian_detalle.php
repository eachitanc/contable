...
FROM
    ...
    INNER JOIN `seg_ctb_retencion_tipo` 
        ON (`seg_ctb_retenciones`.`id_retencion_tipo` = `seg_ctb_retencion_tipo`.`id_retencion_tipo`)
WHERE (`seg_ctb_retencion_tipo`.`id_retencion_tipo` =2 AND  `seg_ctb_doc`.`fecha` BETWEEN '$fecha_inicial' AND '$fecha_corte');
";
    $res = $cmd->query($sql);
    ...
    <div class="px-2 " style="width:90% !important;margin: 0 auto;">
        ...
        foreach ($retenciones as $tp) {
            ...
            foreach ($pagos_realizados as $pr) {
                ...
                $sql = "SELECT distinct cuenta from seg_ctb_libaux WHERE debito >0 and id_ctb_doc = {$pr['id_ctb_doc']}";
                $res = $cmd->query($sql);
