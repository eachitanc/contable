        ...
        foreach ($cuentas as $cta) {
            try {
                $sql = "SELECT
                            ...
                            , `seg_ctb_doc`.`id_manu`
                            , IF(LENGTH(`seg_ctb_libaux`.`id_tercero`) >=1,`seg_ctb_libaux`.`id_tercero`,`seg_ctb_doc`.`id_tercero`) AS id_terceroapi
                            , `seg_ctb_doc`.`detalle`
                        ...
                        WHERE (`seg_ctb_doc`.`fecha` BETWEEN '$fecha_inicial' AND '$fecha_corte'
                            AND `seg_ctb_libaux`.`cuenta` ='{$cta['cuenta']}')
            ...
            try {
                $sql = "SELECT
                        ...
                        FROM
                            ...
                            INNER JOIN `seg_ctb_doc` 
                                ON (`seg_ctb_libaux`.`id_ctb_doc` = `seg_ctb_doc`.`id_ctb_doc`)
                        WHERE (`seg_ctb_doc`.`fecha` < '$fecha_inicial' AND  `seg_ctb_libaux`.`cuenta`='{$cta['cuenta']}')
                        ;";
                $res = $cmd->query($sql);
                $saldo_ini = $res->fetch();
            } catch (PDOException $e) {
