...
try {
    $sql = "SELECT DISTINCT
            ...
            WHERE (SUBSTRING(`seg_ctb_libaux`.`cuenta`,1,$long_ini) BETWEEN $cuenta_ini AND $cuenta_fin);";
    $res = $cmd->query($sql);
        ...
        foreach ($cuentas as $cta) {
            try {
                $sql = "SELECT
                            ...
                            , `seg_ctb_doc`.`id_manu`
                            , IF(`seg_ctb_libaux`.`id_tercero`,`seg_ctb_libaux`.`id_tercero`,`seg_ctb_doc`.`id_tercero`) AS id_terceroapi
                            , `seg_ctb_doc`.`detalle`
                        ...
                        FROM
                            ...
                            INNER JOIN `seg_ctb_doc` 
                                ON (`seg_ctb_libaux`.`id_ctb_doc` = `seg_ctb_doc`.`id_ctb_doc`)
                        WHERE (`seg_ctb_doc`.`fecha` BETWEEN '$fecha_inicial' AND '$fecha_corte'
                            AND `seg_ctb_libaux`.`cuenta` ='{$cta['cuenta']}' AND `seg_ctb_doc`.`estado`=1 )
