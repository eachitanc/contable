...
try {
    $sql = "SELECT cuenta,SUM(debitoi) AS debitoi, SUM(creditoi) AS creditoi,SUM(debito) AS debito,SUM(credito) AS credito FROM (
        SELECT
            ...
            , seg_ctb_libaux.debito AS debitoi
            , seg_ctb_libaux.credito AS creditoi
            , 0 AS debito
            , 0 AS credito
        FROM seg_ctb_libaux
            INNER JOIN seg_ctb_doc ON (seg_ctb_libaux.id_ctb_doc = seg_ctb_doc.id_ctb_doc)
        WHERE seg_ctb_doc.fecha <= '$fecha_inicial' AND seg_ctb_doc.estado=1
        UNION ALL
        SELECT
             seg_ctb_libaux.cuenta
            , 0 AS debitoi
        ...
        FROM seg_ctb_libaux
            INNER JOIN seg_ctb_doc ON (seg_ctb_libaux.id_ctb_doc = seg_ctb_doc.id_ctb_doc)
        WHERE seg_ctb_doc.fecha BETWEEN '$fecha_inicial' AND '$fecha_corte' AND seg_ctb_doc.estado=1
        UNION ALL
