...
try {
    ...
    WHERE (`seg_pto_mvto`.`id_pto_doc` ='$id_pto_doc'
                ...
                <table id="tableContrtacionRpRubros" class="table table-striped table-bordered table-sm table-hover shadow" style="width: 100%;">
                    ...
                    <tbody>
                        ...
                        foreach ($rubros as $ce) {
                            ...
                            $sql = "SELECT sum(valor) as saldo,id_ctb_doc FROM seg_pto_mvto WHERE rubro = '$ce[rubro]' AND (tipo_mov = 'COP' OR tipo_mov = 'LCO') AND id_pto_doc = $ce[id_pto_doc]";
                            $rs = $cmd->query($sql);
                            $saldo = $rs->fetch();
                            $obligado = $saldo['saldo'];
                            $id_ctb_doc = $saldo['id_ctb_doc'];
                            $sq3 = "SELECT sum(valor) as comprom FROM seg_pto_mvto WHERE rubro = '$ce[rubro]' AND tipo_mov = 'CRP' AND id_pto_doc = $ce[id_pto_doc]";
                            $rs3 = $cmd->query($sq3);
                            $com = $rs3->fetch();
                            $comprometido = $com['comprom'];
                            // Consultar el valor liquidado del registro de la tabla seg_pto_mvto
                            $sq3 = "SELECT sum(valor) as liquidado FROM seg_pto_mvto WHERE rubro = '$ce[rubro]' AND tipo_mov = 'LRP' AND id_auto_crp = $ce[id_pto_doc]";
                            $rs3 = $cmd->query($sq3);
                            $liq = $rs3->fetch();
