...
try {
    ...
    $sql = "SELECT
                `id_retroactivo`, `fec_inicio`, `fec_final`, `meses`, `porcentaje`, `id_incremento`, `observaciones`, `seg_retroactivos`.`vigencia`,`seg_retroactivos`.`estado`
            FROM
            `seg_retroactivos`
            ...
            FROM 
                ...
                FROM `seg_empleado`
                    ...
                    (SELECT
                        ...
                        , SUM(`seg_liq_incap`.`dias_liq`) AS `dias_incap`
                    FROM
                        `seg_liq_incap`
                        INNER JOIN `seg_incapacidad` 
                        ON (`seg_liq_incap`.`id_incapacidad` = `seg_incapacidad`.`id_incapacidad`)
                    WHERE (`seg_liq_incap`.`id_nomina` IN($ids_nominas) AND `id_empleado` IN ($ids))
                    GROUP BY `seg_incapacidad`.`id_empleado`) AS `t1`
                        ON (`seg_empleado`.`id_empleado` = `t1`.`id_empleado`)
                    LEFT JOIN
                    (SELECT
                        `seg_licenciasnr`.`id_empleado`
                        , SUM(`seg_liq_licnr`.`dias_licnr`) AS `dias_licnr`
                    FROM
                        `seg_liq_licnr`
                        INNER JOIN `seg_licenciasnr` 
                        ON (`seg_liq_licnr`.`id_licnr` = `seg_licenciasnr`.`id_licnr`)
                        INNER JOIN `seg_nominas` 
                        ON (`seg_liq_licnr`.`id_nomina` = `seg_nominas`.`id_nomina`)
                    WHERE (`seg_liq_licnr`.`id_nomina` IN ($ids_nominas) AND `id_empleado` IN ($ids))
                    GROUP BY `seg_licenciasnr`.`id_empleado`) AS `t2`
                        ON (`seg_empleado`.`id_empleado` = `t2`.`id_empleado`)
                    LEFT JOIN
                    (SELECT
                        `seg_licenciasmp`.`id_empleado`
                        , SUM(`seg_liq_licmp`.`dias_liqs`) AS `dias_licr`
                    FROM
                        `seg_liq_licmp`
                        INNER JOIN `seg_licenciasmp` 
                        ON (`seg_liq_licmp`.`id_licmp` = `seg_licenciasmp`.`id_licmp`)
                        INNER JOIN `seg_nominas` 
                        ON (`seg_liq_licmp`.`id_nomina` = `seg_nominas`.`id_nomina`)
                    WHERE (`seg_liq_licmp`.`id_nomina` IN ($ids_nominas) AND `id_empleado` IN ($ids))
                    GROUP BY `seg_licenciasmp`.`id_empleado`) AS `t3`
                        ON (`seg_empleado`.`id_empleado` = `t3`.`id_empleado`)
                    LEFT JOIN 
                    (SELECT
                        `seg_vacaciones`.`id_empleado`
                        , SUM(`seg_liq_vac`.`dias_liqs`) AS `dias_vac`
                    FROM
                        `seg_liq_vac`
                        INNER JOIN `seg_vacaciones` 
                        ON (`seg_liq_vac`.`id_vac` = `seg_vacaciones`.`id_vac`)
                    WHERE (`seg_liq_vac`.`id_nomina` IN ($idnomvacs) AND `id_empleado` IN ($ids))
                    GROUP BY `seg_vacaciones`.`id_empleado`) AS `t4`
                        ON (`seg_empleado`.`id_empleado` = `t4`.`id_empleado`)
                WHERE `seg_empleado`.`id_empleado` IN ($ids)) AS `t5`";
    $rs = $cmd->query($sql);
...
try {
    ...
    $sql = "SELECT
                ...
                , `seg_horas_ex_trab`.`id_he_trab`
                , `seg_horas_ex_trab`.`id_he`
                , `seg_horas_ex_trab`.`cantidad_he`
                , `seg_tipo_horaex`.`codigo`
                , `seg_tipo_horaex`.`factor`
            FROM
                `seg_liq_horex`
                INNER JOIN `seg_horas_ex_trab` 
                    ON (`seg_liq_horex`.`id_he_lab` = `seg_horas_ex_trab`.`id_he_trab`)
                INNER JOIN `seg_nominas` 
                    ON (`seg_liq_horex`.`id_nomina` = `seg_nominas`.`id_nomina`)
                INNER JOIN `seg_tipo_horaex`
                    ON (`seg_horas_ex_trab`.`id_he` = `seg_tipo_horaex`.`id_he`)
            WHERE (`seg_horas_ex_trab`.`fec_inicio` BETWEEN '$fecIni' AND '$fecFin' AND `seg_liq_horex`.`id_nomina` IN($ids_nominas) AND `id_empleado` IN ($ids))";
    $rs = $cmd->query($sql);
...
try {
    $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
    $sql = "SELECT `id_empleado`, `val_bsp` FROM `seg_liq_bsp` WHERE `id_nomina` IN ($idnomvacs)";
    $rs = $cmd->query($sql);
...
try {
    ...
    $sql = "SELECT
            ...
            FROM
                ...
                INNER JOIN `seg_vacaciones` 
                    ON (`seg_liq_vac`.`id_vac` = `seg_vacaciones`.`id_vac`)
            WHERE `seg_liq_vac`.`id_nomina` IN ($idnomvacs)";
    $rs = $cmd->query($sql);
...
try {
    ...
    $sql = "SELECT
                `seg_incapacidad`.`id_empleado`
                , `seg_incapacidad`.`id_incapacidad`
                , `seg_liq_incap`.`id_eps`
                , `seg_liq_incap`.`id_arl`
                , `seg_liq_incap`.`dias_liq`
...
try {
    ...
    $sql = "SELECT
                ...
                , `seg_indemniza_vac`.`id_indemniza`
...
try {
    ...
    $sql = "SELECT
            ...
            FROM
                `seg_liq_prima_nav`
            WHERE (`id_liq_privac` IN (SELECT MAX(`id_liq_privac`) FROM `seg_liq_prima_nav` WHERE (`id_nomina` IN ($idnomvacs) AND `id_empleado` IN ($ids))
            GROUP BY `id_empleado`))";
    $rs = $cmd->query($sql);
...
try {
    ...
    $sql = "SELECT
            ...
            FROM
                `seg_liq_cesantias`
            WHERE (`id_liq_cesan` IN (SELECT MAX(`id_liq_cesan`) FROM `seg_liq_cesantias` WHERE (`id_nomina` IN ($idnomvacs) AND `id_empleado` IN ($ids))
            GROUP BY `id_empleado`))";
    $rs = $cmd->query($sql);
...
try {
    ...
    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
    $sql = "SELECT
                `seg_licenciasmp`.`id_licmp`
                , `seg_licenciasmp`.`id_empleado`
                , `seg_liq_licmp`.`id_eps`
                , `seg_liq_licmp`.`dias_liqs`
                , `seg_liq_licmp`.`val_liq`
                , `seg_liq_licmp`.`id_nomina`
            FROM
                `seg_liq_licmp`
                INNER JOIN `seg_licenciasmp` 
                    ON (`seg_liq_licmp`.`id_licmp` = `seg_licenciasmp`.`id_licmp`)
            WHERE (`seg_liq_licmp`.`id_nomina` IN ($ids_nominas))";
    $rs = $cmd->query($sql);
...
try {
    $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
    $sql = "SELECT `id_nomina` FROM `seg_nominas` WHERE `id_incremento` = $id_retroactivo AND `tipo` = 'RA' LIMIT 1";
    $rs = $cmd->query($sql);
...
if (count($empleado) > 0) {
    ...
    if (empty($nomina_anterior)) {
        ...
        try {
            $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
            $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
            $sql = "INSERT INTO `seg_nominas` (`tipo`, `vigencia`, `descripcion`,`fec_reg`, `mes`, `id_user_reg`, `id_incremento`) 
                VALUES (?, ?, ?, ?, ?, ?, ?)";
            $sql = $cmd->prepare($sql);
        ...
        try {
            $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
            $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
            $sql = "SELECT `id_empleado` FROM `seg_liq_salario` WHERE `id_nomina` = $id_nomina";
            $rs = $cmd->query($sql);
    ...
    foreach ($empleado as $e) {
        ...
        if (false === $key) {
            ...
            try {
                $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
                $sql = "INSERT INTO `seg_liq_prestaciones_sociales` (`id_empleado`, `val_vacacion`, `val_cesantia`, `val_interes_cesantia`
            ...
            if ($keyindem !== false) {
                ...
                try {
                    $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
                    $sql = "INSERT INTO `seg_liq_indemniza_vac`
            ...
            if ($key !== false) {
                ...
                try {
                    $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
                    $sql = "INSERT INTO `seg_liq_licmp` (`id_licmp`, `id_eps`, `dias_liqs`, `val_liq`, `anio_lic`, `fec_reg`, `id_nomina`) 
                        VALUES (?, ?, ?, ?, ?, ?, ?)";
                    $sql = $cmd->prepare($sql);
            ...
            if (false !== $key) {
                ...
                if ($bsp > 0) {
                    try {
                        $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                        $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
                        $sql = "INSERT INTO `seg_liq_bsp`(`id_empleado`, `val_bsp`, `id_user_reg`, `fec_reg`, `id_nomina`, `anio`) 
                            VALUES (?, ?, ?, ?, ?, ?)";
                        $sql = $cmd->prepare($sql);
                        $sql->bindParam(1, $id, PDO::PARAM_INT);
            ...
            if ($e['fec_retiro'] != '' && $e['fec_retiro'] <= $fecFin) {
                ...
                $doceavas_ants = (($prima_nav_ant * 360) / $diasnov) - $ant_base;
                ...
                $doceavas_ces = (($cesantia_ant * 360) / $diasnov) - $ant_base;
