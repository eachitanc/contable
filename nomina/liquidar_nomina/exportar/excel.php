...
try {
    ...
    FROM seg_liq_salario,seg_meses
    WHERE  seg_liq_salario.mes = seg_meses.codigo AND mes = '$mes' AND anio = '$anio'";
    $rs = $cmd->query($sql);
    $saln = $rs->fetchAll();
    $cmd = null;
} catch (PDOException $e) {
...
try {
    $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
    $sql = "SELECT `id_empleado`,`val_ret` FROM `seg_retencion_fte` WHERE mes = '$mes' AND anio = '$anio'";
    $rs = $cmd->query($sql);
    $ret_fte = $rs->fetchAll();
    $cmd = null;
} catch (PDOException $e) {
