...
try {
    ...
    $sql = "SELECT id_contrato, seg_empleado.id_empleado, no_documento, descripcion_carg, CONCAT(nombre1, ' ', nombre2, ' ', apellido1, ' ', apellido2) AS nombre, fec_inicio, fec_fin, vigencia, tot_dias_lab, tot_dias_vac, sal_base, aux_transp, val_prima, val_cesantias, val_icesantias, val_vacaciones 
            FROM
...
try {
    $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
    $sql = "SELECT seg_liq_vac.id_vac, id_contrato, dias_habiles
            FROM
                seg_liq_vac
...
try {
    ...
    $sql = "SELECT id_empleado, SUM(val_liq_ps) AS tot_prima 
            FROM 
                (SELECT * FROM seg_liq_prima WHERE anio = '$vigencia') AS t
