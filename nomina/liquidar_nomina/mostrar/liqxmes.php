...
try {
    ...
    $sql = "SELECT id_contrato, seg_empleado.id_empleado, no_documento, CONCAT(nombre1, ' ', nombre2, ' ', apellido1, ' ', apellido2) AS nombre, fec_inicio, fec_fin, vigencia, tot_dias_lab, tot_dias_vac, sal_base, aux_transp, val_prima, val_cesantias, val_icesantias, val_vacaciones
            FROM
                seg_liq_contrato_emp
            INNER JOIN seg_contratos_empleados 
                ON (seg_liq_contrato_emp.id_contrato = seg_contratos_empleados.id_contrato_emp)
            INNER JOIN seg_empleado 
                ON (seg_contratos_empleados.id_empleado = seg_empleado.id_empleado)
...
try {
    $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
    $sql = "SELECT seg_liq_vac.id_vac, id_contrato, dias_habiles, anio_vac
            FROM
                seg_liq_vac
...
try {
    ...
    $sql = "SELECT id_empleado, SUM(val_liq_ps) AS tot_prima 
            FROM 
                (SELECT * FROM seg_liq_prima WHERE anio = '$vigencias') AS t
...
try {
    ...
    $sql = "SELECT periodo, anio FROM seg_liq_prima GROUP BY periodo HAVING anio = $vigencias ORDER BY periodo ASC";
    $rs = $cmd->query($sql);
...
try {
    $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
    $sql = "SELECT periodo, anio FROM seg_liq_prima_nav GROUP BY periodo HAVING anio = $vigencias ORDER BY periodo ASC";
    $rs = $cmd->query($sql);
