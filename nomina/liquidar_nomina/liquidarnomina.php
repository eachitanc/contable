...
try {
    ...
    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
    $sql = "SELECT * 
            FROM (SELECT id_empleado, id_eps, fec_afiliacion FROM seg_novedades_eps
...
try {
    ...
    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
    $sql = "SELECT * 
            FROM (SELECT id_empleado, id_arl, fec_afiliacion FROM seg_novedades_arl
            ORDER BY fec_afiliacion DESC) AS t 
            GROUP BY id_empleado";
    $rs = $cmd->query($sql);
    $arl = $rs->fetchAll();
...
try {
    ...
    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
    $sql = "SELECT * 
            FROM (SELECT id_empleado, id_afp, fec_afiliacion FROM seg_novedades_afp
            ORDER BY fec_afiliacion DESC) AS t 
            GROUP BY id_empleado";
    $rs = $cmd->query($sql);
    $afp = $rs->fetchAll();
...
try {
    ...
    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
    $sql = "SELECT * 
            FROM (SELECT id_empleado, id_contrato_emp, fec_inicio FROM seg_contratos_empleados
            WHERE estado = '0'
...
try {
    $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
    $sql = "SELECT id_empleado, salario_integral, no_documento, CONCAT(nombre1, ' ', nombre2, ' ',apellido1, ' ', apellido2) AS nombre FROM seg_empleado
            WHERE  estado = '1'";
    $rs = $cmd->query($sql);
    $emple = $rs->fetchAll();
    $cmd = null;
...
if (isset($_REQUEST['check'])) {
    ...
    foreach ($list_liquidar as $i) {
        ...
        $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
        $sql = "SELECT * FROM seg_liq_salario
        ...
        if (empty($nomliq)) {
            ...
            foreach ($licencias as $lc) {
                if (intval($i) == intval($lc['id_empleado'])) {
                    ...
                    try {
                        $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                        $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
                        $sql = "INSERT INTO seg_liq_licmp (id_licmp, id_eps, fec_inicio, fec_fin, dias_liqs, val_liq, val_dialc, mes_lic, anio_lic, fec_reg) VALUES (?, ?, ?, ?, ?, ?,?, ?, ?, ?)";
                        $sql = $cmd->prepare($sql);
            ...
            foreach ($licenciasnr as $lcnr) {
                if (intval($i) == intval($lcnr['id_empleado'])) {
                    ...
                    try {
                        $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                        $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
                        $sql = "INSERT INTO seg_liq_licnr (id_licnr, fec_inicio, fec_fin, dias_licnr, mes_licnr, anio_licnr, fec_reg) VALUES (?, ?, ?, ?, ?, ?,?)";
                        $sql = $cmd->prepare($sql);
            ...
            foreach ($vacaciones as $vs) {
                if (intval($i) == intval($vs['id_empleado'])) {
                    ...
                    try {
                        $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                        $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
                        $sql = "INSERT INTO seg_liq_vac (id_vac, id_contrato, fec_inicio, fec_fin, dias_liqs, val_liq, val_diavac, mes_vac, anio_vac, fec_reg) VALUES (?, ?, ?, ?, ?,?, ?, ?, ?, ?)";
                        $sql = $cmd->prepare($sql);
            ...
            foreach ($incapacidades as $inc) {
                ...
                if ($emple_inc == $i) {
                    ...
                    try {
                        $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                        $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
                        $sql = "INSERT INTO seg_liq_incap (id_incapacidad, id_eps, id_arl, fec_inicio, fec_fin, dias_liq, pago_empresa, pago_eps, pago_arl, mes, anios, fec_reg) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                        $sql = $cmd->prepare($sql);
            ...
            try {
                $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
                $sql = "INSERT INTO seg_liq_dias_lab (id_empleado, id_contrato, cant_dias, mes, anio, fec_reg) VALUES (?, ?, ?, ?, ?, ?)";
                $sql = $cmd->prepare($sql);
            ...
            try {
                $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
                $sql = "INSERT INTO seg_liq_segsocial_empdo (id_empleado, id_eps, id_arl, id_afp, aporte_salud_emp, aporte_pension_emp, aporte_solidaridad_pensional, porcentaje_ps, aporte_salud_empresa, aporte_pension_empresa, aporte_rieslab, mes, anio, fec_reg) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                $sql = $cmd->prepare($sql);
                $sql->bindParam(1, $i, PDO::PARAM_INT);
                $sql->bindParam(2, $id_eps, PDO::PARAM_INT);
                $sql->bindParam(3, $id_arl, PDO::PARAM_INT);
            ...
            try {
                $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
                $sql = "INSERT INTO seg_liq_prestaciones_sociales (id_empleado, id_contrato, val_vacacion, val_cesantia, val_interes_cesantia, val_prima, mes_prestaciones, anio_prestaciones, fec_reg) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
                $sql = $cmd->prepare($sql);
                $sql->bindParam(1, $empleado, PDO::PARAM_INT);
                $sql->bindParam(2, $id_contrato, PDO::PARAM_INT);
            ...
            try {
                $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
                $sql = "INSERT INTO seg_retencion_fte (id_empleado, val_ret, mes, anio, id_user_reg, fec_reg) VALUES (?, ?, ?, ?, ?, ?)";
                $sql = $cmd->prepare($sql);
                $sql->bindParam(1, $i, PDO::PARAM_INT);
            ...
            try {
                $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
                $sql = "INSERT INTO seg_liq_salario (id_empleado, val_liq, forma_pago, metodo_pago, mes, anio, fec_reg) VALUES (?, ?, ?, ?, ?, ?, ?)";
                $sql = $cmd->prepare($sql);
                $sql->bindParam(1, $i, PDO::PARAM_INT);
