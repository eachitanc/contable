...
try {
    ...
    $sql = "SELECT
            ...
            WHERE `seg_novedades_arl`.`id_novarl`
                IN(SELECT MAX(`id_novarl`) AS `id_novarl` FROM `seg_novedades_arl` WHERE SUBSTRING(`fec_afiliacion`, 1, 4)<= '$anio' GROUP BY `id_empleado`)";
    $rs = $cmd->query($sql);
    $riesgos = $rs->fetchAll();
    $cmd = null;
} catch (PDOException $e) {
...
try {
    $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
    $sql = "SELECT `id_empleado`, `val_pagoxdep` FROM `seg_pago_dependiente`";
    $rs = $cmd->query($sql);
...
try {
    $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
    $sql = "SELECT `id_nomina` FROM `seg_nominas` WHERE `mes` = '$mes' AND `vigencia` = '$anio' AND `tipo` = 'N'";
    $rs = $cmd->query($sql);
...
try {
    ...
    $sql = "SELECT
            ...
            FROM
                `seg_liq_prima`
            WHERE `id_liq_prima` IN (SELECT MAX(`id_liq_prima`) FROM `seg_liq_prima` GROUP BY `id_empleado`)";
    $res = $cmd->query($sql);
...
try {
    ...
    $sql = "SELECT
        ...
        FROM
            `seg_liq_bsp`
        WHERE `id_bonificaciones` IN ( SELECT MAX(`id_bonificaciones`) FROM `seg_liq_bsp` GROUP BY `id_empleado`)";
    $res = $cmd->query($sql);
...
try {
    ...
    $sql = "SELECT 
            ...
            FROM
                ...
                FROM
                    `seg_liq_bsp`
                WHERE `id_bonificaciones` IN (SELECT MAX(`id_bonificaciones`) AS `id_bsp` FROM `seg_liq_bsp` GROUP BY `id_empleado`)) AS `t1`
            INNER JOIN 
                (SELECT
                    `id_empleado`, `cant_dias`, CONCAT(`anio`, `mes`) AS `compara`
                FROM
                    `seg_liq_dias_lab`) AS `t2`
                    ON (`compara` > `corte`)
...
try {
    $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
    $sql = "SELECT `id_param`, `condicion` FROM `seg_parametros`";
    $rs = $cmd->query($sql);
    $parametros = $rs->fetchAll();
    $cmd = null;
} catch (PDOException $e) {
    echo $e->getCode() == 2002 ? 'Sin Conexión a Mysql (Error: 2002)' : 'Error: ' . $e->getMessage();
}
$condHoras = $condBSP = false;
foreach ($parametros as $pm) {
    if ($pm['id_param'] == 1) {
        $condHoras = $pm['condicion'];
    }
    if ($pm['id_param'] == 2) {
        $condBSP = $pm['condicion'];
    }
}
$dossml = $smmlv * 2;
...
if (isset($_POST['check'])) {
    ...
    if (empty($id_nom)) {
        ...
        try {
            $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
            $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
            $sql = "INSERT INTO `seg_nominas` (`mes`, `vigencia`, `descripcion`, `fec_reg`, `id_user_reg`) VALUES (?, ?, ?, ?, ?)";
            $sql = $cmd->prepare($sql);
    ...
    foreach ($list_liquidar as $i) {
        $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
        $sql = "SELECT * FROM `seg_liq_salario` WHERE `mes` = '$mes' AND `anio` = '$anio' AND `id_empleado` = $i";
        $rs = $cmd->query($sql);
        $nomliq = $rs->fetch();
        $cmd = null;
        ...
        if (empty($nomliq)) {
            ...
            if ($key !== false) {
                ...
                try {
                    $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
                    $sql = "INSERT INTO `seg_liq_licluto` (`id_licluto`, `fec_inicio`, `fec_fin`, `dias_licluto`, `mes_licluto`, `anio_licluto`, `fec_reg`,`id_nomina`,`val_liq`) 
                        VALUES (?, ?, ?, ?, ?, ?,?,?,?)";
                    $sql = $cmd->prepare($sql);
            ...
            foreach ($licenciasnr as $lcnr) {
                if (intval($i) == intval($lcnr['id_empleado'])) {
                    ...
                    if (intval($diflcnr) > 0) {
                        ...
                        $finlicnr = $fec_f;
                        $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                        $sql = "UPDATE seg_licenciasnr SET  fec_inicio = ? WHERE id_licnr = ?";
                        $sql = $cmd->prepare($sql);
                    ...
                    try {
                        $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                        $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
                        $sql = "INSERT INTO seg_liq_licnr (id_licnr, fec_inicio, fec_fin, dias_licnr, mes_licnr, anio_licnr, fec_reg,id_nomina) 
                                VALUES (?, ?, ?, ?, ?, ?,?,?)";
                        $sql = $cmd->prepare($sql);
                ...
                foreach ($horas as $h) {
                    if ($h['id_empleado'] == $i) {
                        ...
                        try {
                            $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                            $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
                            $sql = "INSERT INTO seg_liq_horex (id_he_lab, val_liq, mes_he, anio_he, fec_reg, id_nomina) 
                                    VALUES (?, ?, ?, ?, ?, ?)";
                            $sql = $cmd->prepare($sql);
            ...
            foreach ($licencias as $lc) {
                if (intval($i) == intval($lc['id_empleado'])) {
                    ...
                    try {
                        $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                        $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
                        $sql = "INSERT INTO seg_liq_licmp (id_licmp, id_eps, fec_inicio, fec_fin, dias_liqs, val_liq, val_dialc, mes_lic, anio_lic, fec_reg,id_nomina) 
                                VALUES (?, ?, ?, ?, ?, ?,?, ?, ?, ?,?)";
                        $sql = $cmd->prepare($sql);
            ...
            if ($keyindem !== false) {
                ...
                try {
                    ...
                    if ($cmd->lastInsertId() > 0) {
                        $estado = 2;
                        $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                        $sql = "UPDATE `seg_indemniza_vac` SET  `estado` = ? WHERE `id_indemniza` = ?";
                        $sql = $cmd->prepare($sql);
                        $sql->bindParam(1, $estado, PDO::PARAM_INT);
                ...
                foreach ($incapacidades as $inc) {
                    ...
                    if ($emple_inc == $i) {
                        ...
                        try {
                            $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                            $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
                            $sql = "INSERT INTO seg_liq_incap (id_incapacidad, id_eps, id_arl, dias_liq, pago_empresa, pago_eps, pago_arl, mes, anios, fec_reg, id_nomina) 
                                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                            $sql = $cmd->prepare($sql);
            ...
            try {
                $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
                $sql = "INSERT INTO seg_liq_dias_lab (id_empleado, cant_dias, mes, anio, fec_reg,id_nomina) 
                        VALUES (?, ?, ?, ?, ?, ?)";
                $sql = $cmd->prepare($sql);
            ...
            try {
                $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
                $sql = "INSERT INTO seg_liq_dlab_auxt (id_empleado, dias_liq, val_liq_dias, val_liq_auxt,aux_alim,g_representa,horas_ext, mes_liq, anio_liq, fec_reg, id_nomina) 
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                $sql = $cmd->prepare($sql);
                $sql->bindParam(1, $empleado, PDO::PARAM_INT);
                ...
                foreach ($tienembg as $te) {
                    ...
                    if ($base_descuentos > $dctoemb && $base_descuentos > $smmlv) {
                        ...
                        try {
                            $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                            $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
                            $sql = "INSERT INTO seg_liq_embargo (id_embargo, val_mes_embargo, mes_embargo, anio_embargo, fec_reg, id_nomina) 
                                    VALUES (?, ?, ?, ?, ?, ?)";
                            $sql = $cmd->prepare($sql);
            ...
            if ($idcuotsind > 0) {
                ...
                if ($status_sind['estado'] == 1) {
                    ...
                    try {
                        $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                        $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
                        $sql = "UPDATE `seg_cuota_sindical` SET `estado` = ?, `fec_act` = ? WHERE `id_cuota_sindical` = ?";
                        $sql = $cmd->prepare($sql);
            ...
            if ($base_descuentos > $valcuotsind && $base_descuentos > $smmlv) {
                if ($idcuotsind != 0) {
                    try {
                        $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                        $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
                        $sql = "INSERT INTO seg_liq_sindicato_aportes (id_cuota_sindical, val_aporte, mes_aporte, anio_aporte, fec_reg, id_nomina) 
                            VALUES (?, ?, ?, ?, ?, ?)";
                        $sql = $cmd->prepare($sql);
                ...
                foreach ($libranzas as $libranza) {
                    ...
                    if ($base_descuentos > $abonolib && $base_descuentos > $smmlv) {
                        try {
                            $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                            $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
                            $sql = "INSERT INTO seg_liq_libranza (id_libranza, val_mes_lib, mes_lib, anio_lib, fec_reg, id_nomina) 
                                VALUES (?, ?, ?, ?, ?, ?)";
                            $sql = $cmd->prepare($sql);
            ...
            try {
                $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
                $sql = "INSERT INTO seg_retencion_fte (id_empleado, val_ret, mes, anio, id_user_reg, fec_reg, id_nomina,base) 
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
                $sql = $cmd->prepare($sql);
                $sql->bindParam(1, $i, PDO::PARAM_INT);
