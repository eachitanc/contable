...
try {
    ...
    $sql = " SELECT id_empleado, id_contrato, SUM(cant_dias) AS tot_dias FROM 
                (SELECT id_empleado, id_contrato, cant_dias FROM seg_liq_dias_lab WHERE anio = '$vigencia') AS t
        ...
        if (false !== $key) {
            ...
            if (false !== $key) {
                ...
                try {
                    $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
                    $sql = "INSERT INTO seg_liq_contrato_emp (id_contrato, sal_base, aux_transp, val_cesantias, val_icesantias, val_vacaciones, val_prima, tot_dias_lab, tot_dias_vac, id_user_reg, fec_reg) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                    $sql = $cmd->prepare($sql);
                    ...
                    if ($cmd->lastInsertId() > 0) {
                        try {
                            $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                            $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
                            $sql = "UPDATE seg_contratos_empleados SET estado = ?, id_user_act = ?, fec_act = ? WHERE id_contrato_emp = ?";
                            $sql = $cmd->prepare($sql);
