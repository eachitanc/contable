...
try {
    $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
    $sql = "SELECT id_empleado, anio
            FROM seg_liq_cesantias
            WHERE anio = '$anio'";
    $rs = $cmd->query($sql);
    $censatias = $rs->fetchAll();
    $cmd = null;
} catch (PDOException $e) {
    echo $e->getCode() == 2002 ? 'Sin Conexión a Mysql (Error: 2002)' : 'Error: ' . $e->getMessage();
}
...
if (isset($_REQUEST['check'])) {
    ...
    foreach ($list_liquidar as $i) {
        ...
        if ($sal_integ != 1) {
            $key = array_search($i, array_column($censatias, 'id_empleado'));
            if (false === $key) {
                ...
                $totdays = $diaxsem + 30;
                $censantias = $salbase + $gasrep +  $auxtrans + $auxali + $bsp / 12 + $primservicio / 12 + $primvacacion / 12 + $primanavidad / 12;
                $cesantia = ($censantias / 360) * $totdays; //=
                ...
                try {
                    $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
                    $sql = "INSERT INTO `seg_liq_cesantias` (`id_empleado`, `cant_dias`,`val_cesantias`, `val_icesantias`, `porcentaje_interes`, `corte`, `anio`, `fec_reg`)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
                    $sql = $cmd->prepare($sql);
...
$er .= '</tbody>
        ...
        <center><a class="btn btn-link" href="detalles_cesantias.php">Detalles</a></center>';
if ($liquidados == 0) {
    echo '0';
} else {
    echo $er;
