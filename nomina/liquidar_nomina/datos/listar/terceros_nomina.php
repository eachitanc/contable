...
try {
    ...
    $sql = "SELECT
                ...
                , `t1`.`nit` 
                , `t1`.`categoria`
                , `seg_categoria_tercero`.`codigo`
                , `seg_categoria_tercero`.`descripcion`
            FROM 
                (SELECT
                ...
                FROM
                    `seg_sindicatos`) AS `t1` 
            INNER JOIN `seg_categoria_tercero`
                ON (`t1`.`categoria` = `seg_categoria_tercero`.`id_cat`)";
    $rs = $cmd->query($sql);
