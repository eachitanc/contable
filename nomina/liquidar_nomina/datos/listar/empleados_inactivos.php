...
try {
    ...
    $sql = "SELECT
            ...
            WHERE `fec_retiro` <= '$corte' AND `fec_retiro` >= '2023-01-01'
                AND `id_empleado` NOT IN (SELECT `id_empleado` FROM `seg_empleados_retirados`) AND `estado` = 0
            ORDER BY `nombre` ASC";
    $rs = $cmd->query($sql);
    $obj = $rs->fetchAll();
    $cmd = null;
...
try {
    $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
    $sql = "SELECT `id_empleado`,`dias` FROM `seg_liq_compesatorio` WHERE `id_nomina` = 0";
    $rs = $cmd->query($sql);
