...
try {
    $sql = "SELECT
            ...
            WHERE (`seg_vacaciones`.`id_vac` = '$id_vac')";
    $res = $cmd->query($sql);
...
try {
    $sql = "SELECT
            ...
            FROM
                `seg_salarios_basico`
            WHERE  `id_salario` = (SELECT MAX(`id_salario`) FROM `seg_salarios_basico` WHERE `id_empleado` = $datos[id_empleado])";
    $res = $cmd->query($sql);
...
try {
    $sql = "SELECT
            ...
            FROM
                `seg_liq_prima`
            WHERE `id_liq_prima` IN ( SELECT MAX(`id_liq_prima`) FROM `seg_liq_prima` WHERE `id_empleado` = $datos[id_empleado])";
    $res = $cmd->query($sql);
...
try {
    $sql = "SELECT
        ...
        FROM
            `seg_liq_bsp`
        WHERE `id_bonificaciones` IN ( SELECT MAX(`id_bonificaciones`) FROM `seg_liq_bsp` WHERE `id_empleado` = $datos[id_empleado])";
    $res = $cmd->query($sql);
...
//prima de servicios
if ($prima['val_liq_ps'] > 0 && !empty($prima)) {
    $primservicio = $prima['val_liq_ps'];
} else {
    $primservicio = 0;
};
if ($bpserv['val_bsp'] > 0 && !empty($bpserv)) {
    $bsp = $bpserv['val_bsp'];
} else {
    $bsp = 0;
};
