...
try {
    ...
    $sql = "UPDATE `seg_resolucion_viaticos` SET `fec_inicia` = ?, `fec_final` = ?, `tot_dias` = ?, `dias_pernocta` = ?, `objetivo` = ?, `destino` = ? WHERE `id_resol_viat` = ?";
    $sql = $cmd->prepare($sql);
    ...
    if (!($sql->execute())) {
        ...
        if ($sql->rowCount() > 0) {
            $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
            $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
            $sql = "UPDATE `seg_resolucion_viaticos` SET  `id_user_act` = ? ,`fec_act` = ?  WHERE `id_resol_viat` = ?";
            $sql = $cmd->prepare($sql);
