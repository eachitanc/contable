...
if (isset($_POST['check'])) {
    ...
    try {
        ...
        $query = "SELECT `grupo` FROM `seg_consecutivo` WHERE `id_consec` = '1'";
        $rs = $cmd->query($query);
        $consec = $rs->fetch();
        $grupo = $consec['grupo'] + 1;
        $query = "UPDATE `seg_consecutivo` SET `grupo` = `grupo` + 1 WHERE `id_consec` = 1";
        $query = $cmd->prepare($query);
        $query->execute();
        $sql = "INSERT INTO `seg_resolucion_viaticos` (`id_empleado`, `no_resolucion`, `fec_inicia`, `fec_final`, `tot_dias`, `dias_pernocta`, `objetivo`, `destino`, `grupo`, `vigencia`, `id_user_reg`, `fec_reg`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $sql = $cmd->prepare($sql);
        ...
        foreach ($lista_ids as $id) {
            $query = "SELECT `resolucion` FROM `seg_consecutivo` WHERE `id_consec` = '1'";
            $rs = $cmd->query($query);
            $consec = $rs->fetch();
            $no_resolucion = $consec['resolucion'] + 1;
            $query = "UPDATE `seg_consecutivo` SET `resolucion` = `resolucion` + 1 WHERE `id_consec` = 1";
            $query = $cmd->prepare($query);
            $query->execute();
            $id_empleado = $id;
