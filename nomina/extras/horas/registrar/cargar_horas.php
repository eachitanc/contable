...
try {
    ...
    $sql = "SELECT id_empleado, no_documento FROM seg_empleado";
    $rs = $cmd->query($sql);
...
if (!empty($empleados)) {
    if (file_exists('horas_extra.xlsx')) {
        ...
        try {
            $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
            $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
            $sql = "INSERT INTO seg_horas_ex_trab (id_empleado, id_he, fec_inicio, fec_fin, hora_inicio, hora_fin, cantidad_he, fec_reg, tipo) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
            $sql = $cmd->prepare($sql);
