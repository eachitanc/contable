...
try {
    ...
    $sql = "SELECT no_documento FROM seg_empleado WHERE no_documento = '$cc'";
    $rs = $cmd->query($sql);
    if ($rs->rowCount() > 0) {
        echo '0';
    } else {
        $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
        $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
        $sql = "INSERT INTO seg_empleado(tipo_empleado, subtipo_empleado, alto_riesgo_pension, tipo_contrato, tipo_doc
            ...
            if ($cmd->lastInsertId() > 0) {
                ...
                if ($cmd->lastInsertId() > 0) {
                    ...
                    if ($cmd->lastInsertId() > 0) {
                        $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                        $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
                        $sql = "INSERT INTO `seg_novedades_fc`
                        ...
                        if (isset($_POST['checkDependientes'])) {
                            ...
                            $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                            $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
                            $sql = "INSERT INTO `seg_pago_dependiente`
                        ...
                        if ($cmd->lastInsertId() > 0) {
                            try {
                                $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                                $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
                                $sql = "INSERT INTO seg_contratos_empleados (id_empleado, fec_inicio, vigencia, id_user_reg, fec_reg) VALUES (?, ?, ?, ?, ?)";
                                $sql = $cmd->prepare($sql);
                                $sql->bindParam(1, $idinsert, PDO::PARAM_INT);
                                ...
                                if ($cmd->lastInsertId() > 0) {
                                    ...
                                    if ($res > 1 || $regAtTerc = 'SI') {
                                        try {
                                            ...
                                            if ($cmd->lastInsertId() > 0) {
                                                $tipo_tercero = 1;
                                                $sql = "INSERT INTO `rel_tipo_tercero` (`id_tercero_api`, `id_tipo_tercero`, `id_user_reg`, `fec_reg`) VALUES (?, ?, ?, ?)";
                                                $sql = $cmd->prepare($sql);
