...
try {
    ...
    if (isset($_POST['id_salario'])) {
        ...
        $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
        $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
        $sql = "UPDATE seg_salarios_basico SET  salario_basico = ?  WHERE id_salario = ?";
        $sql = $cmd->prepare($sql);
    ...
    if (isset($_POST['checkDependientes'])) {
        ...
        $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
        $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
        $sql = "DELETE FROM  `seg_pago_dependiente` WHERE `id_empleado` = ?";
        $sql = $cmd->prepare($sql);
        $sql->bindParam(1, $idemp, PDO::PARAM_INT);
        $sql->execute();
        $dpt = $sql->rowCount() > 0 ? 1 : 0;
    ...
    if ($updatemp > 0 || $upsalemp > 0 || $dpt > 0) {
        ...
        if ($upsalemp > 0) {
            $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
            $sql = "UPDATE seg_salarios_basico SET  fec_act = ? WHERE WHERE id_salario = ?";
            $sql = $cmd->prepare($sql);
            $sql->bindValue(1, $date->format('Y-m-d H:i:s'));
            $sql->bindParam(2, $id_salario, PDO::PARAM_INT);
            $sql->execute();
