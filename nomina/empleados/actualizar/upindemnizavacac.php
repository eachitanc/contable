...
try {
    ...
    $sql = "UPDATE `seg_indemniza_vac` SET `fec_inica` = ?, `fec_fin` = ?,  `cant_dias` = ?  WHERE `id_indemniza` = ?";
    $sql = $cmd->prepare($sql);
    ...
    if ($updata > 0) {
        $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
        $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
        $sql = "UPDATE `seg_indemniza_vac` SET  `id_user_act` = ?, `fec_act` = ? WHERE `id_indemniza` = ?";
        $sql = $cmd->prepare($sql);
