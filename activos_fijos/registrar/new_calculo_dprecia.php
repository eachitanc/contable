...
try {
    ...
    $sql = "SELECT `id_serial`  FROM `seg_liq_depreciacion` WHERE `mes` = '$mes' AND `anio` = '$vigencia'";
    $rs = $cmd->query($sql);
...
try {
    $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
    $sql = "INSERT INTO `seg_liq_depreciacion` (`id_serial`, `dias`, `val_depreciado`, `fecha`, `mes`, `anio`,  `id_user_reg`, `fec_reg`)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    $sql = $cmd->prepare($sql);
