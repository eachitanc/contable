...
try {
    ...
    $sql = "SELECT
                ...
                , `seg_sedes_empresa`.`direcccion`
                , `seg_num_serial`.`tipo_entra`
                , `seg_num_serial`.`recomendaciones`
                , `seg_centros_costo` .`descripcion` AS `centro_costo`
            FROM
                ...
                INNER JOIN `seg_centro_costo_x_sede` 
                    ON (`seg_ubica_traslado_centro_costo`.`id_centro_costo` = `seg_centro_costo_x_sede`.`id_x_sede`)
                INNER JOIN `seg_estado_acfijo` 
                    ON (`seg_ubica_traslado_centro_costo`.`estado` = `seg_estado_acfijo`.`id_estado`)
                ...
                INNER JOIN `seg_departamento` 
                    ON (`seg_sedes_empresa`.`id_dpto` = `seg_departamento`.`id_dpto`)
                INNER JOIN `seg_municipios` 
                    ON (`seg_municipios`.`id_departamento` = `seg_departamento`.`id_dpto`) AND (`seg_sedes_empresa`.`id_municipio` = `seg_municipios`.`id_municipio`)
    ...
    $sql = "SELECT
            ...
            FROM
                ...
                INNER JOIN `seg_bien_servicio` 
                    ON (`seg_entra_detalle_activos_fijos`.`id_prod` = `seg_bien_servicio`.`id_b_s`)
...
try {
    ...
    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
    $sql = "SELECT 
                `id_compra`, `fec_ini` FROM `seg_contrato_compra` 
                                ...
                                <div class="overflow px-1">
                                    <table class="w-100 table-bordered table-sm text-left" style="font-size:85%; white-space: nowrap;">
                                        <tr>
                                            ...
                                            <td><?php echo $obj_hv['nombre_dpto'] ?></td>
                                    ...
                                    <table class="w-100 table-bordered table-sm text-left" style="font-size:85%; white-space: nowrap;">
                                        <form id="formHVEquipoGral">
                                            ...
                                            <tr>
                                                ...
                                                <td class="px-2">
                                                    <div class="px-0_5"><?php echo $obj_hv['sede'] . ' ' . $obj_hv['centro_costo'] . ', ' . $obj_hv['direcccion'] . ' ' . $obj_hv['nom_municipio'] . ' (' . $obj_hv['nombre_dpto'] . ')' ?></div>
                                                </td>
                                            </tr>
                                            <tr>
