...
try {
    $sql = "SELECT `codigo`,`nom_mes`,`fin_mes` FROM `seg_meses` WHERE `codigo` = $mes";
    $res = $cmd->query($sql);
    $dataMes = $res->fetch();
} catch (PDOException $e) {
    echo $e->getCode() == 2002 ? 'Sin Conexión a Mysql (Error: 2002)' : 'Error: ' . $e->getCode();
...
try {
    $sql = "SELECT
                ...
                , `seg_liq_depreciacion`.`dias`
                , `seg_liq_depreciacion`.`val_depreciado`
                , `seg_liq_depreciacion`.`fecha`
                , `seg_liq_depreciacion`.`mes`
                , `seg_liq_depreciacion`.`anio`
            FROM
                `seg_liq_depreciacion`
                INNER JOIN `seg_num_serial` 
                    ON (`seg_liq_depreciacion`.`id_serial` = `seg_num_serial`.`id_serial`)
            ...
            WHERE (`seg_liq_depreciacion`.`mes` = '$mes' AND `seg_liq_depreciacion`.`anio` = '$vigencia')
            ORDER BY `seg_tipo_bien_servicio`.`tipo_bn_sv` ASC, `seg_bien_servicio`.`bien_servicio` ASC";
    $res = $cmd->query($sql);
