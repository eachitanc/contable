...
try {
    ...
    WHERE ((`seg_pto_mvto`.`tipo_mov` ='ADI' OR `seg_pto_mvto`.`tipo_mov`='RED' OR `seg_pto_mvto`.`tipo_mov`='TRA' OR `seg_pto_mvto`.`tipo_mov`='APL') AND `seg_pto_documento`.`fecha` <= '$fecha_corte' AND `seg_pto_documento`.`estado`=5)
    GROUP BY `seg_pto_mvto`.`tipo_mov`,`seg_pto_mvto`.`rubro`, `seg_pto_mvto`.`id_pto_doc`
    ORDER BY `seg_pto_documento`.`fecha`, `seg_pto_mvto`.`tipo_mov` ASC;
";
    $res = $cmd->query($sql);
