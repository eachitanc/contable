...
try {
    ...
    , IF(seg_pto_cargue.tipo_dato = 1, seg_pto_cargue.ppto_aprob, 0) AS inicial
...
FROM seg_pto_cargue
INNER JOIN seg_pto_presupuestos ON (seg_pto_cargue.id_pto_presupuestos = seg_pto_presupuestos.id_pto_presupuestos)
LEFT JOIN (
	SELECT seg_pto_mvto.rubro,SUM(seg_pto_mvto.valor) AS valor
	FROM seg_pto_mvto
	INNER JOIN seg_pto_cargue ON (seg_pto_mvto.rubro = seg_pto_cargue.cod_pptal)
	INNER JOIN seg_pto_documento ON (seg_pto_mvto.id_pto_doc = seg_pto_documento.id_pto_doc)
	WHERE seg_pto_documento.estado =0 AND seg_pto_documento.fecha BETWEEN '$fecha_ini' AND '$fecha_corte' AND seg_pto_mvto.tipo_mov ='ADI' AND seg_pto_mvto.mov =0
	GROUP BY seg_pto_mvto.rubro
) AS adicion ON (adicion.rubro=seg_pto_cargue.cod_pptal) 
LEFT JOIN (
	SELECT seg_pto_mvto.rubro,SUM(seg_pto_mvto.valor) AS valor
	FROM seg_pto_mvto
	INNER JOIN seg_pto_cargue ON (seg_pto_mvto.rubro = seg_pto_cargue.cod_pptal)
	INNER JOIN seg_pto_documento ON (seg_pto_mvto.id_pto_doc = seg_pto_documento.id_pto_doc)
	WHERE seg_pto_documento.estado =0 AND seg_pto_documento.fecha BETWEEN '$fecha_ini_mes' AND '$fecha_corte' AND seg_pto_mvto.tipo_mov ='ADI' AND seg_pto_mvto.mov =0
	GROUP BY seg_pto_mvto.rubro
) AS adicion_mes ON (adicion_mes.rubro=seg_pto_cargue.cod_pptal)
LEFT JOIN(
	SELECT seg_pto_mvto.rubro,SUM(seg_pto_mvto.valor) AS valor
	FROM seg_pto_mvto
	INNER JOIN seg_pto_cargue ON (seg_pto_mvto.rubro = seg_pto_cargue.cod_pptal)
	INNER JOIN seg_pto_documento  ON (seg_pto_mvto.id_pto_doc = seg_pto_documento.id_pto_doc)
	WHERE seg_pto_documento.estado =0 AND seg_pto_documento.fecha BETWEEN '$fecha_ini' AND' $fecha_corte' AND seg_pto_mvto.tipo_mov ='RED' AND seg_pto_mvto.mov =1
        GROUP BY seg_pto_mvto.rubro
) AS reduccion ON (reduccion.rubro=seg_pto_cargue.cod_pptal)
LEFT JOIN(
	SELECT seg_pto_mvto.rubro,SUM(seg_pto_mvto.valor) AS valor
	FROM seg_pto_mvto
	INNER JOIN seg_pto_cargue ON (seg_pto_mvto.rubro = seg_pto_cargue.cod_pptal)
	INNER JOIN seg_pto_documento  ON (seg_pto_mvto.id_pto_doc = seg_pto_documento.id_pto_doc)
	WHERE seg_pto_documento.estado =0 AND seg_pto_documento.fecha BETWEEN '$fecha_ini_mes'AND'$fecha_corte' AND seg_pto_mvto.tipo_mov ='RED' AND seg_pto_mvto.mov =1 
        GROUP BY seg_pto_mvto.rubro
) AS reduccion_mes ON (reduccion_mes.rubro=seg_pto_cargue.cod_pptal)
LEFT JOIN(
	SELECT seg_pto_mvto.rubro,SUM(seg_pto_mvto.valor) AS valor
	FROM seg_pto_mvto
	INNER JOIN seg_pto_cargue ON (seg_pto_mvto.rubro = seg_pto_cargue.cod_pptal)
	INNER JOIN seg_pto_documento  ON (seg_pto_mvto.id_pto_doc = seg_pto_documento.id_pto_doc)
	WHERE seg_pto_documento.estado =0 AND seg_pto_documento.fecha BETWEEN '$fecha_ini'AND'$fecha_corte' AND seg_pto_mvto.tipo_mov ='TRA' AND seg_pto_mvto.mov =1 
        GROUP BY seg_pto_mvto.rubro
) AS credito ON (credito.rubro=seg_pto_cargue.cod_pptal)
LEFT JOIN(
	SELECT seg_pto_mvto.rubro,SUM(seg_pto_mvto.valor) AS valor
	FROM seg_pto_mvto
	INNER JOIN seg_pto_cargue ON (seg_pto_mvto.rubro = seg_pto_cargue.cod_pptal)
	INNER JOIN seg_pto_documento  ON (seg_pto_mvto.id_pto_doc = seg_pto_documento.id_pto_doc)
	WHERE seg_pto_documento.estado =0 AND seg_pto_documento.fecha BETWEEN '$fecha_ini_mes'AND'$fecha_corte' AND seg_pto_mvto.tipo_mov ='TRA' AND seg_pto_mvto.mov =1 
        GROUP BY seg_pto_mvto.rubro
) AS credito_mes ON (credito_mes.rubro=seg_pto_cargue.cod_pptal)
LEFT JOIN(
	SELECT seg_pto_mvto.rubro,SUM(seg_pto_mvto.valor) AS valor
	FROM seg_pto_mvto
	INNER JOIN seg_pto_cargue ON (seg_pto_mvto.rubro = seg_pto_cargue.cod_pptal)
	INNER JOIN seg_pto_documento  ON (seg_pto_mvto.id_pto_doc = seg_pto_documento.id_pto_doc)
	WHERE seg_pto_documento.estado =0 AND seg_pto_documento.fecha BETWEEN '$fecha_ini'AND'$fecha_corte' AND seg_pto_mvto.tipo_mov ='TRA' AND seg_pto_mvto.mov =0 
        GROUP BY seg_pto_mvto.rubro
) AS contracredito ON (contracredito.rubro=seg_pto_cargue.cod_pptal)
LEFT JOIN(
	SELECT seg_pto_mvto.rubro,SUM(seg_pto_mvto.valor) AS valor
	FROM seg_pto_mvto
	INNER JOIN seg_pto_cargue ON (seg_pto_mvto.rubro = seg_pto_cargue.cod_pptal)
	INNER JOIN seg_pto_documento  ON (seg_pto_mvto.id_pto_doc = seg_pto_documento.id_pto_doc)
	WHERE seg_pto_documento.estado =0 AND seg_pto_documento.fecha BETWEEN '$fecha_ini_mes'AND'$fecha_corte' AND seg_pto_mvto.tipo_mov ='TRA' AND seg_pto_mvto.mov =0 
        GROUP BY seg_pto_mvto.rubro
) AS contracredito_mes ON (contracredito_mes.rubro=seg_pto_cargue.cod_pptal)
LEFT JOIN(
	SELECT seg_pto_mvto.rubro,SUM(seg_pto_mvto.valor) AS valor
	FROM seg_pto_mvto
	INNER JOIN seg_pto_cargue ON (seg_pto_mvto.rubro = seg_pto_cargue.cod_pptal)
	INNER JOIN seg_pto_documento  ON (seg_pto_mvto.id_pto_doc = seg_pto_documento.id_pto_doc)
	WHERE seg_pto_documento.estado =0 AND seg_pto_documento.fecha BETWEEN '$fecha_ini'AND'$fecha_corte' AND (seg_pto_mvto.tipo_mov ='CDP' OR seg_pto_mvto.tipo_mov ='LCD') AND seg_pto_mvto.mov =0 
        GROUP BY seg_pto_mvto.rubro
) AS compromiso_cdp ON (compromiso_cdp.rubro=seg_pto_cargue.cod_pptal)
LEFT JOIN(
	SELECT seg_pto_mvto.rubro,SUM(seg_pto_mvto.valor) AS valor
	FROM seg_pto_mvto
	INNER JOIN seg_pto_cargue ON (seg_pto_mvto.rubro = seg_pto_cargue.cod_pptal)
	INNER JOIN seg_pto_documento  ON (seg_pto_mvto.id_pto_doc = seg_pto_documento.id_pto_doc)
	WHERE seg_pto_documento.estado =0 AND seg_pto_documento.fecha BETWEEN '$fecha_ini_mes'AND'$fecha_corte' AND (seg_pto_mvto.tipo_mov ='CDP' OR seg_pto_mvto.tipo_mov ='LCD') AND seg_pto_mvto.mov =0 
        GROUP BY seg_pto_mvto.rubro
) AS compromiso_cdp_mes ON (compromiso_cdp_mes.rubro=seg_pto_cargue.cod_pptal)
LEFT JOIN(
	SELECT seg_pto_mvto.rubro,SUM(seg_pto_mvto.valor) AS valor
	FROM seg_pto_mvto
	INNER JOIN seg_pto_cargue ON (seg_pto_mvto.rubro = seg_pto_cargue.cod_pptal)
	INNER JOIN seg_pto_documento  ON (seg_pto_mvto.id_pto_doc = seg_pto_documento.id_pto_doc)
	WHERE seg_pto_documento.estado =0 AND seg_pto_documento.fecha BETWEEN '$fecha_ini'AND'$fecha_corte' AND (seg_pto_mvto.tipo_mov ='CRP' OR seg_pto_mvto.tipo_mov ='LRP') AND seg_pto_mvto.mov =0 
        GROUP BY seg_pto_mvto.rubro
) AS compromiso_crp ON (compromiso_crp.rubro=seg_pto_cargue.cod_pptal)
LEFT JOIN(
	SELECT seg_pto_mvto.rubro,SUM(seg_pto_mvto.valor) AS valor
	FROM seg_pto_mvto
	INNER JOIN seg_pto_cargue ON (seg_pto_mvto.rubro = seg_pto_cargue.cod_pptal)
	INNER JOIN seg_pto_documento  ON (seg_pto_mvto.id_pto_doc = seg_pto_documento.id_pto_doc)
	WHERE seg_pto_documento.estado =0 AND seg_pto_documento.fecha BETWEEN '$fecha_ini_mes'AND'$fecha_corte' AND (seg_pto_mvto.tipo_mov ='CRP' OR seg_pto_mvto.tipo_mov ='LRP')  AND seg_pto_mvto.mov =0 
        GROUP BY seg_pto_mvto.rubro
) AS compromiso_crp_mes ON (compromiso_crp_mes.rubro=seg_pto_cargue.cod_pptal)
LEFT JOIN(
	SELECT `seg_pto_mvto`.`rubro`, SUM(`seg_pto_mvto`.`valor`) AS valor
	FROM `seg_ctb_doc`
	INNER JOIN `seg_pto_mvto` ON (`seg_ctb_doc`.`id_ctb_doc` = `seg_pto_mvto`.`id_ctb_doc`)
	WHERE `seg_ctb_doc`.`estado`=1 AND `seg_ctb_doc`.`fecha`  BETWEEN '$fecha_ini' AND '$fecha_corte' AND `seg_pto_mvto`.`tipo_mov` ='COP'
	GROUP BY seg_pto_mvto.rubro
) AS obligacion ON (obligacion.rubro=seg_pto_cargue.cod_pptal)
LEFT JOIN(
	SELECT `seg_pto_mvto`.`rubro`, SUM(`seg_pto_mvto`.`valor`) AS valor
	FROM `seg_ctb_doc`
	INNER JOIN `seg_pto_mvto` ON (`seg_ctb_doc`.`id_ctb_doc` = `seg_pto_mvto`.`id_ctb_doc`)
	WHERE `seg_ctb_doc`.`estado`=1 AND `seg_ctb_doc`.`fecha`  BETWEEN '$fecha_ini_mes' AND '$fecha_corte' AND `seg_pto_mvto`.`tipo_mov` ='COP'
	GROUP BY seg_pto_mvto.rubro
) AS obligacion_mes ON (obligacion_mes.rubro=seg_pto_cargue.cod_pptal)
LEFT JOIN(
	SELECT `seg_pto_mvto`.`rubro`, SUM(`seg_pto_mvto`.`valor`) AS valor
	FROM `seg_ctb_doc`
	INNER JOIN `seg_pto_mvto` ON (`seg_ctb_doc`.`id_ctb_doc` = `seg_pto_mvto`.`id_ctb_doc`)
	WHERE `seg_ctb_doc`.`estado`=1 AND `seg_ctb_doc`.`fecha`  BETWEEN '$fecha_ini' AND '$fecha_corte' AND `seg_pto_mvto`.`tipo_mov` ='PAG'
	GROUP BY seg_pto_mvto.rubro
) AS pagos ON (pagos.rubro=seg_pto_cargue.cod_pptal)
LEFT JOIN(
	SELECT `seg_pto_mvto`.`rubro`, SUM(`seg_pto_mvto`.`valor`) AS valor
	FROM `seg_ctb_doc`
	INNER JOIN `seg_pto_mvto` ON (`seg_ctb_doc`.`id_ctb_doc` = `seg_pto_mvto`.`id_ctb_doc`)
	WHERE `seg_ctb_doc`.`estado`=1 AND `seg_ctb_doc`.`fecha`  BETWEEN '$fecha_ini_mes' AND '$fecha_corte' AND `seg_pto_mvto`.`tipo_mov` ='PAG'
	GROUP BY seg_pto_mvto.rubro
) AS pagos_mes ON (pagos_mes.rubro=seg_pto_cargue.cod_pptal)
WHERE seg_pto_cargue.vigencia =2023 AND seg_pto_presupuestos.id_pto_tipo =2 
