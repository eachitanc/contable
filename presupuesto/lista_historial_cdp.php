...
try {
    ...
    INNER JOIN `seg_pto_documento` 
        ON (`seg_pto_mvto`.`id_pto_doc` = `seg_pto_documento`.`id_pto_doc`)
    WHERE (`seg_pto_mvto`.`id_auto_dep` ='$cdp' AND tipo_mov ='CRP')
...
try {
    $sql = "SELECT
            ...
            FROM
                ...
                INNER JOIN `seg_pto_documento` 
                    ON (`seg_pto_mvto`.`id_pto_doc` = `seg_pto_documento`.`id_pto_doc`)
            WHERE (`seg_pto_mvto`.`id_auto_dep` =$cdp AND tipo_mov ='LCD');";
    $res = $cmd->query($sql);
...
try {
    $sql = "SELECT
            ...
            FROM
                ...
                INNER JOIN `seg_pto_documento` 
                    ON (`seg_pto_mvto`.`id_pto_doc` = `seg_pto_documento`.`id_pto_doc`)
            WHERE (`seg_pto_mvto`.`id_auto_dep` =$cdp AND tipo_mov ='LRP');";
    $res = $cmd->query($sql);
            ...
            <table id="tableListaCrp" class="table table-striped table-bordered  table-sm table-hover " style="width: 100%;">
                ...
                <tbody>
                    ...
                    foreach ($crp as $lp) {
                        ...
                        try {
                            $sql = "SELECT
                                            SUM(`seg_pto_mvto`.`valor`) as val_causado
                                        , `seg_pto_mvto`.`id_pto_doc`
                                    FROM
                                        `seg_pto_mvto`
                                        INNER JOIN `seg_pto_documento` 
