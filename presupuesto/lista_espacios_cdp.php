...
try {
    ...
    $sql = "SELECT MIN(id_manu) AS minimo, MAX(id_manu) AS mayor FROM seg_pto_documento WHERE fecha > '$fecha_cierre' AND tipo_doc= 'CDP'";
    $rs = $cmd->query($sql);
    $rangos = $rs->fetch();
        ...
        <div class="px-3">
            <table id="tableListaCdp" class="table table-striped table-bordered  table-sm table-hover " style="width: 100%;">
                ...
                <tbody>
                    ...
                    $estado = '';
                    do {
                        $sql = "SELECT * FROM seg_pto_documento WHERE id_manu ='$id_manu' ORDER BY id_manu ASC";
                        $res = $cmd->query($sql);
