...
try {
    ...
    $sql = "SELECT id_pto_presupuestos,fecha, id_manu,objeto,id_auto FROM seg_pto_documento WHERE id_pto_doc=$id_pto_documento";
    $rs = $cmd->query($sql);
    $datosCrp = $rs->fetch();
    $cmd = null;
...
try {
    $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
    $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
    $sql = "SELECT sum(valor) as valorCrp FROM seg_pto_mvto WHERE id_pto_doc=$datosCrp[id_auto]";
    $rs = $cmd->query($sql);
    $totalCrp = $rs->fetch();
