...
if ($presupuesto == 1) {
    ...
    try {
        ...
        if ($ingreso == 0) {
            $sql = "INSERT INTO `seg_pto_homologa_ingresos`
                (`id_pto`, `id_cgr`, `id_cpc`, `id_fuente`, `id_tercero`, `id_politica`, `id_siho`, `id_sia`, `id_situacion`, `id_vigencia`, `id_user_reg`, `fec_reg`)
            VALUES (?, ?, ?, ? , ?, ?, ?, ?, ?, ?, ?, ?)";
        } else {
            $sql = "UPDATE `seg_pto_homologa_ingresos` 
                SET `id_pto` = ?, `id_cgr` = ?, `id_cpc` = ?, `id_fuente` = ?, `id_tercero` = ?, `id_politica` = ?, `id_siho` = ?, `id_sia` = ?, `id_situacion` = ?, `id_vigencia` = ?
        ...
        foreach ($codCgrs as $key => $value) {
            ...
            if ($sql->rowCount() > 0) {
                ...
                if ($ingreso == 1) {
                    ...
                    $query = "UPDATE `seg_pto_homologa_ingresos` SET `id_user_act` = ?, `fec_act` = ? WHERE `id_homologacion` = ?";
    ...
    try {
        ...
        $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
        if ($gasto == 0) {
            $sql = "INSERT INTO `seg_pto_homologa_gastos`
                        (`id_pto`, `id_cgr`, `id_cpc`, `id_fuente`, `id_tercero`, `id_politica`, `id_siho`, `id_sia`, `id_situacion`, `id_vigencia`, `id_seccion`, `id_sector`, `id_csia`, `id_user_reg`, `fec_reg`)
                    VALUES (?, ?, ?, ? , ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        } else {
            $sql = "UPDATE `seg_pto_homologa_gastos` 
                    SET `id_pto` = ?, `id_cgr` = ?, `id_cpc` = ?, `id_fuente` = ?, `id_tercero` = ?, `id_politica` = ?, `id_siho` = ?, `id_sia` = ?, `id_situacion` = ?, `id_vigencia` = ?, `id_seccion` = ?, `id_sector` = ?, `id_csia` = ?
                    WHERE `id_homologacion` = ?";
        }
        $sql = $cmd->prepare($sql);
        $sql->bindParam(1, $id_ppto, PDO::PARAM_INT);
        ...
        foreach ($codCgrs as $key => $value) {
            ...
            if ($sql->rowCount() > 0) {
                ...
                if ($gasto == 1) {
                    $con = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
                    $con->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
                    $query = "UPDATE `seg_pto_homologa_gastos` SET `id_user_act` = ?, `fec_act` = ? WHERE `id_homologacion` = ?";
                    $query = $con->prepare($query);
                    $query->bindParam(1, $iduser, PDO::PARAM_INT);
                    $query->bindValue(2, $date->format('Y-m-d H:i:s'));
                    $query->bindParam(3, $idHom, PDO::PARAM_INT);
