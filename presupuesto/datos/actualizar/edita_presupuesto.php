...
try {
    ...
    $sql = "SELECT nombre,id_pto_tipo,descripcion FROM seg_pto_presupuestos WHERE id_pto_presupuestos = '$id'";
    $rs = $cmd->query($sql);
    $presupuesto = $rs->fetch();
    // Consulto tipo de presupuesto
    $sql = "SELECT id_pto_tipo,nombre FROM seg_pto_tipo ORDER BY nombre ASC";
    $rs = $cmd->query($sql);
        ...
        <form id="formUpdatePresupuesto">
                    ...
                    <select id="tipoPto" name="tipoPto" class="form-control form-control-sm py-0 sm" aria-label="Default select example">
                        ...
                        foreach ($modalidad as $mo) {
                            if($mo['id_pto_tipo'] == $presupuesto['id_pto_tipo']){
                                echo '<option value="'.$mo['id_pto_tipo'].'" selected>'.$mo['nombre'].'</option>';
                            }else{
            ...
            <div class="form-row px-4 pt-2">
                ...
                <div class="text-center pb-3">
                    <button class="btn btn-primary btn-sm" id="btnUpdatePresupuesto">Agregar</button>
