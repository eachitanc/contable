...
try {
    $sql = "SELECT codigo_doc, nombre, version_doc, fecha_doc FROM seg_fin_maestro_doc WHERE tipo_doc ='CDP' AND estado=0";
    $res = $cmd->query($sql);
    $datos = $res->fetch();
...
try {
    $sql = "SELECT nombre FROM seg_empresas WHERE nit ='$_SESSION[nit_emp]'";
    $res = $cmd->query($sql);
    $empresa = $res->fetch();
} catch (PDOException $e) {
    echo $e->getCode() == 2002 ? 'Sin Conexión a Mysql (Error: 2002)' : 'Error: ' . $e->getCode();
...
try {
    $sql = "SELECT objeto,fecha,id_manu FROM seg_pto_documento WHERE id_pto_doc ='$_post[id]'";
    $res = $cmd->query($sql);
    $cdp = $res->fetch();
} catch (PDOException $e) {
    echo $e->getCode() == 2002 ? 'Sin Conexión a Mysql (Error: 2002)' : 'Error: ' . $e->getCode();
...
try {
    ...
    , `seg_pto_cargue`.`vigencia` as vigencia
    FROM
    `seg_pto_mvto`
    INNER JOIN `seg_pto_cargue` 
        ON (`seg_pto_mvto`.`rubro` = `seg_pto_cargue`.`cod_pptal`)
    WHERE `seg_pto_mvto`.`id_pto_doc` =$_post[id];
    ";
    $res = $cmd->query($sql);
    $rubros = $res->fetchAll();
} catch (PDOException $e) {
