...
// Buscamos si hay registros posteriores a la fecha recibida
    $sql = "SELECT max(id_manu) as id_manu from seg_pto_documento WHERE CAST(fecha AS DATE) <= '$_post[fecha]' AND tipo_doc='$_post[documento]' ";
...
// Buscar espacios libre hacia adelante
    ...
    do {
        $id_manu = $id_manu + 1; 
        $sql = "SELECT id_manu from seg_pto_documento WHERE id_manu='$id_manu' AND tipo_doc='$_post[documento]' ";
        $res = $conexion->query($sql);
        $num = $res->num_rows;
        if($num == 0) {
            $i = 2;
