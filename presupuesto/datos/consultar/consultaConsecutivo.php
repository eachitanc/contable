...
$sql = "SELECT count(id_manu) AS id_manu FROM seg_pto_documento WHERE YEAR(fecha) = '$_SESSION[vigencia]' AND tipo_doc='$_post[documento]'";
...
do {
    $id_manu = $id_manu + 1;
    $sql = "SELECT id_manu from seg_pto_documento WHERE id_manu='$id_manu' AND tipo_doc='$_post[documento]' AND YEAR(fecha) = '$_SESSION[vigencia]'";
    $res = $conexion->query($sql);
    $num = $res->num_rows;
    if ($num == 0) {
        $i = 2;
