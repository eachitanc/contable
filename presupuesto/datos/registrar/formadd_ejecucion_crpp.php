...
try {
    ...
    $sql = "SELECT id_pto_doc,id_pto_presupuestos,objeto,fecha FROM seg_pto_documento WHERE id_pto_doc=$data";
    $rs = $cmd->query($sql);
    $datoscdp = $rs->fetch(PDO::FETCH_ASSOC);
    // Buscar el ultimo max  id_manu de la tabla seg_pto_documento
    $sql = "SELECT max(id_manu) as id_manu FROM seg_pto_documento WHERE tipo_doc='CRP'";
    $rs = $cmd->query($sql);
