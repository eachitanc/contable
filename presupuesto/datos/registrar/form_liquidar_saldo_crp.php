...
$cdp = $_post['id'];
try {
    $sql = "SELECT id,fecha FROM seg_fin_fecha WHERE vigencia = '2023'  ";
    $res = $cmd->query($sql);
    $row = $res->fetch();
    if (!$row) {
        $fecha = date("Y-m-d");
...
try {
    ...
    $sql = "SELECT min(fecha_cierre) as fecha_cierre FROM seg_fin_periodos WHERE vigencia = '2022'";
    $res = $cmd->query($sql);
    $datos = $res->fetch();
...
try {
    ...
    INNER JOIN `seg_pto_documento` 
     ...
     WHERE `seg_pto_mvto`.`id_pto_doc` ='$cdp' AND tipo_mov='CRP';";
    $res = $cmd->query($sql);
                        ...
                        <table id="tableListaCdp" class="table table-striped table-bordered  table-sm table-hover " style="width: 97%;">
                            ...
                            <tbody>
                                ...
                                foreach ($cdps as $lp) {
                                    ...
                                    try {
                                        $sql = "SELECT sum(valor) as registrado FROM seg_pto_mvto WHERE id_pto_doc = '$lp[id_pto_doc]' AND rubro ='$lp[rubro]' AND tipo_mov ='COP'";
                                        $res = $cmd->query($sql);
                                    ...
                                    try {
                                        $sql = "SELECT sum(valor) as liberado FROM seg_pto_mvto WHERE id_auto_crp = '$lp[id_pto_doc]' AND rubro ='$lp[rubro]' AND tipo_mov ='LRP'";
                                        $res = $cmd->query($sql);
