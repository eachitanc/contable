...
if (isset($_POST)) {
    ...
    if (isset($_POST['estado']) && empty($_POST['id_pto_apl'])) {
        ...
        try {
            ...
            $query = $cmd->prepare("INSERT INTO seg_pto_documento (tipo_doc, id_manu,fecha, objeto, tipo_mod,estado ,id_pto_presupuestos,id_user_reg, fec_reg) VALUES (?, ?, ?, ?, ?, ?, ?, ?,?)");
            ...
            $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
            $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
            $query = $cmd->prepare("INSERT INTO seg_pto_mvto (id_pto_doc, tipo_mov, mov, rubro, valor,estado,id_auto_dep) VALUES (?, ?, ?, ?, ?, ?,?)");
    ...
    if (isset($_POST['estado']) && !empty($_POST['id_pto_apl'])) {
        ...
        try {
            $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
            $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
            $query = $cmd->prepare("UPDATE seg_pto_documento SET id_manu =?, tipo_mod = ?, fecha =?, objeto=?,id_user_act=?,fec_act=? WHERE id_pto_doc = ?");
