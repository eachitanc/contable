...
if (isset($_POST)) {
    ...
    if (isset($_POST['id_pto_cdp'])) {
        ...
        try {
            ...
            $query = $cmd->prepare("INSERT INTO seg_pto_documento (id_pto_presupuestos,tipo_doc,id_tercero, id_manu,id_auto,fecha, objeto,num_contrato,estado,id_user_reg, fec_reg) VALUES (?, ?, ?,?, ?, ?, ?, ?, ?,?,?)");
            ...
            $cmd = new PDO("$bd_driver:host=$bd_servidor;dbname=$bd_base;$charset", $bd_usuario, $bd_clave);
            $cmd->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_SILENT);
            $query = $cmd->prepare("INSERT INTO seg_pto_mvto (id_pto_doc, tipo_mov, rubro, valor,id_auto_dep,estado) VALUES (?, ?, ?, ?, ?,?)");
            ...
            foreach ($obj as $key => $value) {
                ...
                $sql = "SELECT id_pto_doc,rubro FROM seg_pto_mvto WHERE id_pto_mvto = $id_cdp";
