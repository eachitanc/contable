...
if (isset($_POST)) {
    ...
    if (empty($_POST['id_pto_mvto'])) {
        ...
        if ($cmd->lastInsertId() > 0) {
            ...
            // Actualizo id_cdp en la tabla seg_adquisiciones
            if (isset($_POST['id_adq'])) {
                $query = $cmd->prepare("UPDATE seg_adquisiciones SET id_cdp = ? WHERE id_adquisicion = ?");
                $query->bindParam(1, $id, PDO::PARAM_INT);
                $query->bindParam(2, $id_adq, PDO::PARAM_INT);
                $query->execute();
            }
            // Actualizo el id_cdp en la tabla seg_novedad_contrato_adi_pror
            if (isset($_POST['id_otro'])) {
                $query = $cmd->prepare("UPDATE seg_novedad_contrato_adi_pror SET cdp = ? WHERE id_nov_con = ?");
                $query->bindParam(1, $id, PDO::PARAM_INT);
        ...
        $query = $cmd->prepare("UPDATE seg_pto_documento SET id_manu = ?, fecha = ?, objeto =?, id_user_act = ?, fec_act=?, num_solicitud=? WHERE id_pto_doc = ?");
