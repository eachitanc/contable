...
if (isset($_POST)) {
    ...
    $valorCdp = $valorCdp * -1;
    try {
        $sql = "SELECT rubro FROM `seg_pto_mvto` WHERE id_pto_mvto = $id_pto_mov";
        $res = $cmd->query($sql);
    ...
    try {
        $sql = "SELECT id_auto_dep FROM `seg_pto_mvto` WHERE id_pto_doc = $id_pto_doc limit 1";
        $res = $cmd->query($sql);
    ...
    if (empty($_POST['editarRubro'])) {
        $query = $cmd->prepare("INSERT INTO seg_pto_mvto (id_pto_doc, tipo_mov, rubro, valor,id_auto_dep,id_auto_crp,estado) VALUES (?, ?, ?, ?,?,?,?)");
        ...
        $query = $cmd->prepare("UPDATE seg_pto_mvto SET rubro =?, valor = ? WHERE id_pto_mvto = ?");
