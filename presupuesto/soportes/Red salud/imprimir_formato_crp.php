...
try {
    $sql = "SELECT objeto,fecha,id_manu,tipo_doc,id_tercero,id_auto,num_contrato FROM seg_pto_documento WHERE id_pto_doc =$dto AND tipo_doc='CRP'";
    $res = $cmd->query($sql);
    $cdp = $res->fetch();
...
try {
    $sql = "SELECT id_manu FROM seg_pto_documento WHERE id_pto_doc =$cdp[id_auto] AND tipo_doc='CDP'";
    $res = $cmd->query($sql);
    $cdp_auto = $res->fetch();
} catch (PDOException $e) {
    echo $e->getCode() == 2002 ? 'Sin Conexión a Mysql (Error: 2002)' : 'Error: ' . $e->getCode();
...
// Valor total del cdp
try {
    $sql = "SELECT sum(valor) as valor FROM seg_pto_mvto WHERE id_pto_doc =$dto AND tipo_mov='CRP'";
    $res = $cmd->query($sql);
    $datos = $res->fetch();
    $total = $datos['valor'];
} catch (PDOException $e) {
    ...
    <div class="px-2 " style="width:90% !important;margin: 0 auto;">
        ...
        <table class="table-bordered bg-light" style="width:100% !important;">
            ...
            foreach ($rubros as $rp) {
                $rubro = $rp['cod_pptal'];
                $sq2 = "SELECT sum(valor) as valor FROM seg_pto_mvto WHERE rubro LIKE '$rubro%' AND id_pto_doc =$dto and tipo_mov='CRP'";
                $res = $cmd->query($sq2);
                $valor = $res->fetch();
                $afecta = $valor['valor'];
                if ($afecta > 0) {
