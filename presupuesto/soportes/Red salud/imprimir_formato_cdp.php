...
try {
    $sql = "SELECT objeto,fecha,id_manu,num_solicitud FROM seg_pto_documento WHERE id_pto_doc =$dto";
    $res = $cmd->query($sql);
    $cdp = $res->fetch();
    ...
    <div class="px-2 " style="width:90% !important;margin: 0 auto;">
        ...
        <table class="table-bordered bg-light" style="width:100% !important;">
            ...
            foreach ($rubros as $rp) {
                $rubro = $rp['cod_pptal'];
                $sql = "SELECT sum(valor) as valor FROM seg_pto_mvto WHERE rubro LIKE '$rubro%' AND id_pto_doc =$dto";
                $res = $cmd->query($sql);
                $valor = $res->fetch();
                $afecta = $valor['valor'];
                if ($afecta > 0) {
